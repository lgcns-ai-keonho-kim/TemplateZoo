...
현재 기본 채팅 그래프(`src/chatbot/core/chat/nodes/*.py`)는 Gemini 설정을 사용한다.
...
5. `ServiceExecutor`가 작업 큐를 소비하며 `start -> token* -> done/error` 이벤트를 보낸다.
6. `ChatService`는 사용자 메시지 저장, 그래프 실행, assistant 응답 영속화를 담당한다.
7. 완료 시 assistant 응답은 `request_id` 멱등 기준으로 1회만 저장된다.
8. 필요하면 `GET /chat/{session_id}`로 최종 스냅샷을 조회한다.
...
| 변수 | 기본값 | 설명 |
| --- | --- | --- |
| `GEMINI_MODEL` | - | 기본 채팅/세이프가드 노드에서 사용할 모델명 |
...
- `.env.sample`에는 `OPENAI_*` 키도 남아 있지만, 현재 기본 그래프는 사용하지 않는다.
- `ENV=dev/stg/prod`를 사용할 때는 `src/chatbot/resources/<env>/.env`가 추가로 필요하다.
...
### 6-2. `src/chatbot/` 디렉토리 레벨 책임 맵

| 경로 | 역할 | 구현 책임 범위 | 책임 밖 범위 |
| --- | --- | --- | --- |
| `src/chatbot/api` | FastAPI 진입 계층 | 라우터 등록, 요청/응답 모델, HTTP 예외 변환, 런타임 의존성 주입 | 도메인 규칙 자체, DB 엔진 세부 구현 |
| `src/chatbot/core` | 채팅 도메인 계층 | 상태 모델, 프롬프트, 노드 조립, 그래프 분기 규칙 | HTTP 프로토콜, 큐/버퍼 인프라 운영 |
| `src/chatbot/shared` | 공용 실행 계층 | 서비스 오케스트레이션, 저장소, 메모리, 런타임 유틸리티, 공통 예외/로깅/설정 | 특정 외부 제품 API 세부 구현 |
| `src/chatbot/integrations` | 외부 연동 계층 | LLM/DB/파일 시스템 어댑터, 연결/매퍼/엔진 | 채팅 유스케이스 정책, API 라우팅 |
| `src/chatbot/resources` | 환경 리소스 계층 | `dev/stg/prod`별 `.env` 파일 보관 | 런타임 로직 |
| `src/chatbot/static` | 웹 UI 정적 리소스 계층 | HTML/CSS/JS, 아이콘, 프런트 상태 처리 | FastAPI 비즈니스 로직, 도메인 정책 |

### 6-3. `src/chatbot/api` 하위 구조
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/chatbot/api/chat/models` | Chat DTO | `/chat` 요청/응답 검증, SSE 구독 입력 모델 |
| `src/chatbot/api/chat/routers` | Chat 라우터 | 작업 제출, 스트림 구독, 세션 스냅샷 조회 |
| `src/chatbot/api/chat/services` | Chat 런타임 조립 | `ChatService`, `ServiceExecutor`, 큐/버퍼 싱글턴 조립 |
| `src/chatbot/api/chat/utils` | Chat API 보조 유틸 | 도메인 모델을 API 응답 형태로 변환 |
| `src/chatbot/api/ui/models` | UI DTO | 세션/메시지 목록 응답 모델 |
| `src/chatbot/api/ui/routers` | UI 라우터 | 세션 생성, 조회, 삭제 엔드포인트 |
| `src/chatbot/api/ui/services` | UI 서비스 | `ChatService` 결과를 UI 응답 모델로 매핑 |
| `src/chatbot/api/ui/utils` | UI 매퍼 | 도메인 엔티티를 UI DTO로 변환 |
| `src/chatbot/api/health` | Health API | `/health` 단일 엔드포인트와 응답 모델 관리 |

### 6-4. `src/chatbot/core/chat` 하위 구조
...
src/chatbot/core/chat/
  ...
  state/                # 그래프 상태 키 정의
  prompts/              # 시스템 프롬프트
  nodes/                # safeguard/response 노드 조립체
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/chatbot/core/chat/const` | 도메인 설정값 | 컬렉션명, 페이지 크기, 기본 문맥 길이, 차단 메시지 |
| `src/chatbot/core/chat/models` | 핵심 엔티티 | 세션, 메시지, 역할, 턴 결과 모델 정의 |
| `src/chatbot/core/chat/state` | 그래프 상태 계약 | 노드 간 전달되는 상태 키의 타입 계약 |
| `src/chatbot/core/chat/prompts` | 프롬프트 정책 | 일반 응답, safeguard 분류 프롬프트 관리 |
| `src/chatbot/core/chat/nodes` | 도메인 노드 조립 | 실제 사용할 LLM/메시지/분기 노드 인스턴스 선언 |
| `src/chatbot/core/chat/graphs` | 그래프 정의 | 노드 연결, 진입점, 조건 분기, stream 정책 설정 |
| `src/chatbot/core/chat/utils` | 매핑 유틸 | 도메인 모델과 저장 문서 간 변환 지원 |

### 6-5. `src/chatbot/shared` 하위 구조
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/chatbot/shared/chat/interface` | 포트 계약 | 그래프, 서비스, 실행기 인터페이스 정의 |
| `src/chatbot/shared/chat/graph` | 그래프 공통 실행기 | 그래프 컴파일, 이벤트 표준화, stream 필터링 |
| `src/chatbot/shared/chat/nodes` | 재사용 노드 | LLMNode, BranchNode, MessageNode 같은 범용 노드 |
| `src/chatbot/shared/chat/repositories` | 대화 이력 저장소 | 세션/메시지 CRUD, request_id 멱등 저장 관리 |
| `src/chatbot/shared/chat/memory` | 세션 메모리 캐시 | 최근 메시지 컨텍스트 메모리 유지 |
| `src/chatbot/shared/chat/services` | 서비스 계층 | `ChatService`, `ServiceExecutor` 실행 오케스트레이션 |
| `src/chatbot/shared/runtime/queue` | 작업 큐 | InMemory/Redis 큐 인터페이스와 구현 |
| `src/chatbot/shared/runtime/buffer` | 이벤트 버퍼 | SSE 이벤트 적재/소비 저장소 |
| `src/chatbot/shared/runtime/worker` | 백그라운드 실행기 | 워커 모델과 실행 루프 지원 |
| `src/chatbot/shared/runtime/thread_pool` | 스레드풀 유틸 | 비동기성 보조 스레드 실행 정책 |
| `src/chatbot/shared/config` | 설정 로딩 | `.env`/JSON/환경 변수 병합 및 런타임 환경 해석 |
| `src/chatbot/shared/logging` | 공통 로깅 | Logger, 로그 모델, 저장소 구현 |
| `src/chatbot/shared/exceptions` | 공통 예외 | 코드/원인 추적이 가능한 애플리케이션 예외 정의 |

### 6-6. `src/chatbot/integrations` 하위 구조
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/chatbot/integrations/llm` | LLM 연동 | LangChain 모델 래핑, 호출 로깅, 예외 표준화 |
| `src/chatbot/integrations/db/base` | DB 공통 타입 | 엔진/세션/모델/쿼리 계약 |
| `src/chatbot/integrations/db/query_builder` | 쿼리 빌더 | 읽기/쓰기/삭제 쿼리 조립 유틸 |
| `src/chatbot/integrations/db/engines/sqlite` | SQLite 엔진 | 기본 Chat 저장소 엔진 |
| `src/chatbot/integrations/db/engines/postgres` | PostgreSQL 엔진 | 관계형 저장 및 벡터 검색 지원 |
| `src/chatbot/integrations/db/engines/mongodb` | MongoDB 엔진 | 문서 저장/조회 어댑터 |
| `src/chatbot/integrations/db/engines/redis` | Redis 엔진 | keyspace 기반 저장/벡터 유틸 |
| `src/chatbot/integrations/db/engines/elasticsearch` | Elasticsearch 엔진 | 인덱스 기반 검색 어댑터 |
| `src/chatbot/integrations/db/engines/lancedb` | LanceDB 엔진 | 로컬 벡터 저장소 지원 |
| `src/chatbot/integrations/fs/base` | 파일 저장소 계약 | 파일 엔진 인터페이스 |
| `src/chatbot/integrations/fs/engines` | 파일 엔진 구현 | 로컬 파일 시스템 저장 구현 |

### 6-7. `src/chatbot/resources`, `static`

| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/chatbot/resources/dev` | 개발 환경 리소스 | `ENV=dev`일 때 추가 로드할 `.env` 보관 |
| `src/chatbot/resources/stg` | 스테이징 환경 리소스 | `ENV=stg`용 설정 보관 |
| `src/chatbot/resources/prod` | 운영 환경 리소스 | `ENV=prod`용 설정 보관 |
| `src/chatbot/static/index.html` | UI 엔트리 | 채팅 화면 골격 |
| `src/chatbot/static/css` | 스타일 | 레이아웃/테마 정의 |
| `src/chatbot/static/js/core` | 프런트 초기화 | 앱 부트스트랩과 전역 흐름 제어 |
| `src/chatbot/static/js/chat` | 채팅 UI | 전송, 셀 렌더링, 스트림 반영 |
| `src/chatbot/static/js/ui` | UI 제어 | 패널 토글, 그리드, 테마 관리 |
| `src/chatbot/static/js/utils` | 프런트 유틸 | DOM, Markdown, 문법 하이라이팅 |
| `src/chatbot/static/asset` | 정적 자산 | 아이콘, 로고 등 시각 리소스 |
