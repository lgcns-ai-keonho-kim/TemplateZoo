...
디자인 패턴: 매니저 패턴
참조: src/base_template/integrations/db/engines/sqlite/vector_store.py
"""
...
class SqliteSchemaManager:
    ...
    def create_collection(self, schema: CollectionSchema) -> None:
        ...
        if vector_field(schema):
            self._vector_store.create_vector_table(schema)
    ...
    def delete_collection(self, name: str) -> None:
        ...
        if self._enable_vector:
            self._vector_store.drop_vector_table_by_collection(name)
    ...
    def add_column(self, schema: CollectionSchema, column: ColumnSpec) -> None:
        ...
        if column.is_vector or column.name == schema.vector_field:
            self._vector_store.create_vector_table(schema, column.name)
            return
        connection = self._connection_provider()
        table = self._identifier.quote_table(schema.name)
    ...
    def drop_column(self, schema: CollectionSchema, column_name: str) -> None:
        ...
        if column_name == schema.vector_field:
            self._vector_store.drop_vector_table(schema)
            return
        connection = self._connection_provider()
        table = self._identifier.quote_table(schema.name)
        cursor = connection.cursor()
