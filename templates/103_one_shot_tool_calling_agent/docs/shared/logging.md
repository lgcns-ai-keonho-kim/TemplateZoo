...
`src/one_shot_tool_calling_agent/shared/logging`의 로깅 인터페이스와 저장소 구현을 코드 기준으로 정리한다.

## 1. 용어 정리

| 용어 | 의미 | 관련 스크립트 |
| --- | --- | --- |
| Logger | 로그 기록 인터페이스 | `_logger_base.py` |
| LogRepository | 로그 저장소 인터페이스 | `_log_repository_interface.py` |
| InMemoryLogger | 기본 로거 구현체 | `_in_memory_logger.py` |
...
| 파일 | 역할 |
| --- | --- |
| `src/one_shot_tool_calling_agent/shared/logging/models.py` | `LogLevel`, `LogContext`, `LogRecord` 정의 |
| `src/one_shot_tool_calling_agent/shared/logging/logger.py` | logging 공개 API 파사드 |
| `src/one_shot_tool_calling_agent/shared/logging/_logger_base.py` | `Logger` 인터페이스 정의 |
| `src/one_shot_tool_calling_agent/shared/logging/_log_repository_interface.py` | `LogRepository` 인터페이스 정의 |
| `src/one_shot_tool_calling_agent/shared/logging/_in_memory_log_repository.py` | 인메모리 저장소 구현 |
| `src/one_shot_tool_calling_agent/shared/logging/_in_memory_logger.py` | `InMemoryLogger` 구현 |
| `src/one_shot_tool_calling_agent/shared/logging/_create_default_logger.py` | 기본 로거 팩토리 함수 |
| `src/one_shot_tool_calling_agent/shared/logging/db_repository.py` | 일반 로그 DB 저장 구현 |
| `src/one_shot_tool_calling_agent/shared/logging/llm_repository.py` | LLM 로그 DB 저장 구현 |
| `src/one_shot_tool_calling_agent/shared/logging/__init__.py` | 공개 API 제공 |
...
| 증상 | 원인 후보 | 확인 스크립트 | 조치 |
| --- | --- | --- | --- |
| 로그가 메모리에만 쌓임 | DB 저장소 미주입 | `logger.py` 파사드 및 `_create_default_logger.py` | repository 주입 경로 확인 |
