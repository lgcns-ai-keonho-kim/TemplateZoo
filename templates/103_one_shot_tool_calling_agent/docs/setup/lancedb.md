...
파일 기반 벡터 저장소로 LanceDB를 사용할 때 필요한 최소 설정과 검증 절차를 정리한다.
...
- 현재 기본 Agent 그래프는 벡터 검색 노드를 포함하지 않습니다.
...
| 경로 | 역할 |
| --- | --- |
| `src/one_shot_tool_calling_agent/integrations/db/engines/lancedb/engine.py` | LanceDB CRUD/벡터 검색 엔진 |
| `src/one_shot_tool_calling_agent/integrations/db/engines/lancedb/schema_adapter.py` | 스키마/차원 어댑터 |
| `src/one_shot_tool_calling_agent/integrations/db/client.py` | 엔진 공통 퍼사드 |
