...
PostgreSQL 설치, pgvector 확장 활성화, 그리고 선택적 DB 엔진 실험 경로를 정리한다.

## 1. 적용 범위

1. PostgreSQL 벡터 검색(`vector`, `ivfflat`) 사용 준비
2. DB 엔진 테스트/실험
3. 수동 조립 코드에서 PostgreSQL DB 클라이언트를 구성하는 예시

주의:

- 현재 기본 `/agent` 런타임은 PostgreSQL 저장소를 자동으로 사용하지 않는다.
- 이 문서는 DB 엔진 테스트 또는 커스텀 런타임 조립 시 참고용이다.

## 2. 관련 코드

| 경로 | 역할 |
| --- | --- |
| `src/one_shot_tool_calling_agent/integrations/db/engines/postgres/engine.py` | PostgreSQL 엔진 |
| `src/one_shot_tool_calling_agent/integrations/db/engines/postgres/connection.py` | psycopg2 연결/pgvector 등록 |
| `src/one_shot_tool_calling_agent/integrations/db/engines/postgres/vector_store.py` | 확장/인덱스 처리 |
| `tests/integrations/db/CRUD/test_postgres_engine_crud.py` | CRUD 검증 |
| `tests/integrations/db/Vector/test_postgres_engine_vector.py` | 벡터 검색 검증 |
...
기본 런타임 대신 별도 DB 클라이언트 조립이 필요할 때만 사용한다.

```python
import os

from one_shot_tool_calling_agent.integrations.db import DBClient
from one_shot_tool_calling_agent.integrations.db.engines.postgres import PostgresEngine
