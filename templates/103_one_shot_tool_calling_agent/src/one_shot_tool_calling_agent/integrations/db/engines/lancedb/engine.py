...
디자인 패턴: 어댑터 패턴
참조: src/one_shot_tool_calling_agent/integrations/db/base/engine.py
"""
...
from one_shot_tool_calling_agent.integrations.db.base.engine import BaseDBEngine
from one_shot_tool_calling_agent.integrations.db.base.models import (
    CollectionSchema,
    ColumnSpec,
    Document,
    Query,
...
from one_shot_tool_calling_agent.integrations.db.engines.lancedb.document_mapper import (
    LanceDocumentMapper,
)
from one_shot_tool_calling_agent.integrations.db.engines.lancedb.filter_engine import (
    LanceFilterEngine,
)
from one_shot_tool_calling_agent.integrations.db.engines.lancedb.schema_adapter import (
    LanceSchemaAdapter,
)
from one_shot_tool_calling_agent.integrations.db.engines.sql_common import (
    ensure_schema,
    vector_field,
)
...
class LanceDBEngine(BaseDBEngine):
    ...
    def add_column(
        ...
        if column.name in table.schema.names:
            return
        table.add_columns(
            self._schema_adapter.build_arrow_field(column, resolved_schema)
        )
    ...
    def upsert(
        ...
        for document in documents:
            ...
            if not raw_row:
                continue
            rows.append(
                self._schema_adapter.normalize_row(
                    raw_row, table.schema, resolved_schema
                )
            )
    ...
    def query(
        ...
        if query.sort or needs_in_memory_filter:
            ...
            if query.pagination:
                builder = builder.offset(query.pagination.offset).limit(
                    query.pagination.limit
                )
            rows = builder.to_arrow().to_pylist()

        documents = [
            self._document_mapper.row_to_document(row, resolved_schema) for row in rows
        ]
        ...
        if query.sort:
            documents = self._filter_engine.apply_sort(
                documents, query, resolved_schema
            )
            if query.pagination:
                start = query.pagination.offset
                end = start + query.pagination.limit
