...
from one_shot_tool_calling_agent.integrations.db.engines.sql_common import ensure_schema
from one_shot_tool_calling_agent.integrations.db.engines.elasticsearch.connection import (
    ElasticConnectionManager,
)
from one_shot_tool_calling_agent.integrations.db.engines.elasticsearch.document_mapper import (
    ElasticDocumentMapper,
)
from one_shot_tool_calling_agent.integrations.db.engines.elasticsearch.filter_builder import (
    ElasticFilterBuilder,
)
from one_shot_tool_calling_agent.integrations.db.engines.elasticsearch.schema_manager import (
...
class ElasticsearchEngine(BaseDBEngine):
    ...
    def drop_column(
        ...
        raise RuntimeError(
            "Elasticsearch는 컬럼 삭제를 지원하지 않습니다. reindex가 필요합니다."
        )
    ...
    def query(
        ...
        filter_query = self._filter_builder.build(
            query.filter_expression, resolved_schema
        )
        if filter_query:
        ...
        if query.sort:
            ...
            for sort_field in query.sort:
                ...
                source = resolved_schema.resolve_source(
                    sort_field.field, sort_field.source
                )
                if source == FieldSource.PAYLOAD:
                    sort_field_name = (
                        f"{resolved_schema.payload_field}.{sort_field.field}"
                    )
                else:
