...
디자인 패턴: 전략 패턴
참조: src/one_shot_tool_calling_agent/shared/config/loader.py, src/one_shot_tool_calling_agent/api/main.py
"""
...
class RuntimeEnvironmentLoader:
    ...
    4. `dev/stg/prod`인 경우 `src/one_shot_tool_calling_agent/resources/<env>/.env`를 로드한다.
    """
    ...
    def _resolve_runtime_env(self) -> str:
        ...
        if normalized not in self._SUPPORTED_ENVS:
            ...
            raise ValueError(
                f"지원하지 않는 ENV 값입니다: {raw_value}. 허용값: {supported_values}"
            )
        return normalized
    ...
    def _resolve_resource_env_file(self, runtime_env: str) -> Path:
        ...
        if candidate.exists():
            return candidate
        raise FileNotFoundError(f"환경 파일을 찾을 수 없습니다: {candidate}")
    ...
    *,
    ...
    try:
        ...
        raise BaseAppException(
            "Gemini 임베딩 차원 설정이 올바르지 않습니다.", detail
        ) from None
