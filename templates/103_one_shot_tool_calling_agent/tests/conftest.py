...
def agent_server_context(
    ...
    tmp_dir = tempfile.TemporaryDirectory(prefix="agent-e2e-")

    command = [
        sys.executable,
        "-m",
        "uvicorn",
        "one_shot_tool_calling_agent.api.main:app",
        "--host",
        "127.0.0.1",
        "--port",
        str(port),
...
@pytest.fixture
def agent_api_client(agent_server_context: AgentServerContext) -> Iterator[httpx.Client]:
    """Agent API 호출용 HTTP 클라이언트를 반환한다."""

    with httpx.Client(base_url=agent_server_context.base_url, timeout=_E2E_CLIENT_TIMEOUT) as client:
        yield client
