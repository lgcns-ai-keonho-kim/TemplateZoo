...
from text_to_sql.shared.chat.services.service_executor import ServiceExecutor
from text_to_sql.shared.exceptions import BaseAppException, ExceptionDetail
from text_to_sql.shared.runtime.buffer import EventBufferConfig, InMemoryEventBuffer
from text_to_sql.shared.runtime.queue import InMemoryQueue, QueueConfig
...
class _SuccessService(_BaseService):
    async def astream(self, session_id: str, user_query: str, context_window: int = 20):
        del session_id, user_query, context_window
...
class _ErrorService(_BaseService):
    async def astream(self, session_id: str, user_query: str, context_window: int = 20):
        ...
        if False:  # pragma: no cover
            yield {"node": "response", "event": "done", "data": ""}
