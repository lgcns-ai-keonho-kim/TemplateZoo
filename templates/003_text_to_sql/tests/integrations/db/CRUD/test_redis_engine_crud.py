...
디자인 패턴: 테스트 케이스
참조: src/text_to_sql/integrations/db/engines/redis/engine.py
"""
...
def test_redis_engine_basic_crud() -> None:
    ...
    if not params:
        raise RuntimeError("REDIS_HOST/REDIS_PORT/REDIS_DB 환경 변수가 필요합니다.")

    _log_step("엔진 생성", mode="direct" if "url" not in params else "url")
    ...
    client.create_collection(_collection_schema(collection))
