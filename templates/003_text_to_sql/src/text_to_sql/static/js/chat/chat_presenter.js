...
(function () {
  ...
  function renderMarkdownInner(text) {
    return window.App.markdown.render(String(text || ''));
  }

  function renderMarkdownBlock(text) {
    return '<div class="markdown">' + renderMarkdownInner(text) + '</div>';
  }

  function renderAssistantHtmlInner(text) {
    var htmlUtil = window.App.html;
    var raw = String(text || '');
    if (!htmlUtil || typeof htmlUtil.sanitizeLimitedHtml !== 'function') {
      return renderMarkdownInner(raw);
    }
    return htmlUtil.sanitizeLimitedHtml(raw);
  }

  function renderAssistantHtmlBlock(text) {
    return '<div class="markdown assistant-html">' + renderAssistantHtmlInner(text) + '</div>';
  }

  window.App.chatPresenter.firstLine = function (text) {
    var raw = String(text || '').trim();
    if (window.App.html && typeof window.App.html.isLikelyHtml === 'function' && window.App.html.isLikelyHtml(raw)) {
      raw = window.App.html.extractPlainText(raw);
    }
    if (!raw) {
      return '';
    }
  ...
  window.App.chatPresenter.renderMessageHtml = function (role, text) {
    var normalizedRole = window.App.chatPresenter.normalizeRole(role);
    if (
      normalizedRole === 'assistant' &&
      window.App.html &&
      typeof window.App.html.isLikelyHtml === 'function' &&
      window.App.html.isLikelyHtml(text)
    ) {
      return renderAssistantHtmlBlock(text);
    }
  ...
  window.App.chatPresenter.renderAssistantBufferedStreamingHtml = function (leadText, withCaret) {
    var leadHtml = renderMarkdownInner(String(leadText || ''));
    var caretHtml = withCaret ? '<span class="typing-caret"></span>' : '';
    var placeholderHtml =
      '<div class="assistant-html-placeholder">' +
      '<p>표 데이터를 정리하고 있습니다.</p>' +
      '</div>';
    return leadHtml + placeholderHtml + caretHtml;
  };

  window.App.chatPresenter.renderAssistantFinalHtml = function (text) {
    if (
      window.App.html &&
      typeof window.App.html.isLikelyHtml === 'function' &&
      window.App.html.isLikelyHtml(text)
    ) {
      return renderAssistantHtmlInner(text);
    }
