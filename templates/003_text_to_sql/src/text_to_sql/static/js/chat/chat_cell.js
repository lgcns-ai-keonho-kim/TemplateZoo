...
(function () {
  ...
  window.App.chatCell.create = function (cellId, title, options) {
    ...
    function addMessage(role, text) {
      var created = presenter.createMessageNode(role, presenter.renderMessageHtml(role, text));
    ...
    function renderStreamingBubble() {
      ...
      var renderFallback = presenter && typeof presenter.renderStreamingHtml === 'function'
        ...
        : null;
      var renderBuffered = presenter && typeof presenter.renderAssistantBufferedStreamingHtml === 'function'
        ? presenter.renderAssistantBufferedStreamingHtml
        : null;

      if (
        state.renderMode !== 'html_buffering' &&
        htmlUtil &&
        typeof htmlUtil.findStreamingHtmlStart === 'function'
      ) {
        var htmlStartIndex = htmlUtil.findStreamingHtmlStart(state.receivedText);
        if (htmlStartIndex >= 0) {
          state.renderMode = 'html_buffering';
          state.streamLeadText = state.receivedText.slice(0, htmlStartIndex).trimEnd();
          state.streamHtmlBuffer = state.receivedText.slice(htmlStartIndex);
        }
      } else if (state.renderMode === 'html_buffering') {
        var knownLeadLength = String(state.streamLeadText || '').length;
        state.streamHtmlBuffer = state.receivedText.slice(knownLeadLength);
      }

      if (state.renderMode === 'html_buffering') {
        if (renderBuffered) {
          state.activeBubble.innerHTML = renderBuffered(state.streamLeadText, true);
        } else if (renderFallback) {
          state.activeBubble.innerHTML = renderFallback(state.streamLeadText, true);
        } else {
          state.activeBubble.textContent = state.streamLeadText;
        }
        scrollMessagesToBottom(isAutoScrollFollowing());
        return;
      }
    ...
    function renderFinalBubble(text) {
      ...
      if (presenter && typeof presenter.renderAssistantFinalHtml === 'function') {
        state.activeBubble.innerHTML = presenter.renderAssistantFinalHtml(text);
      } else if (presenter && typeof presenter.renderStreamingHtml === 'function') {
    ...
    function finishSending() {
      ...
      state.receivedText = '';
      state.renderMode = 'markdown_streaming';
      state.streamLeadText = '';
      state.streamHtmlBuffer = '';
      state.references = [];
