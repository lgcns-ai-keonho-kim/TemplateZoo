...
디자인 패턴: 파사드
참조: src/text_to_sql/integrations/db/base/engine.py, src/text_to_sql/integrations/db/base/query_builder.py
"""
...
from text_to_sql.integrations.db.query_builder.delete_builder import DeleteBuilder
from text_to_sql.integrations.db.query_builder.aggregate_read_builder import (
    AggregateReadBuilder,
)
from text_to_sql.integrations.db.query_builder.read_builder import ReadBuilder
from text_to_sql.integrations.db.query_builder.write_builder import WriteBuilder
...
class DBClient:
    ...
    def add_column(self, collection: str, column: ColumnSpec) -> None:
        ...
        with self._schema_lock:
            current = self._schemas.get(collection) or CollectionSchema.default(
                collection
            )
    ...
    def drop_column(self, collection: str, column_name: str) -> None:
        ...
        with self._schema_lock:
            ...
            updated = current.model_copy(deep=True)
            updated.columns = [
                col for col in updated.columns if col.name != column_name
            ]
    ...
    def aggregate(self, collection: str) -> AggregateReadBuilder:
        """집계 조회 DSL 빌더를 반환한다."""

        return AggregateReadBuilder(
            self._engine,
            collection,
            self.get_schema(collection),
            aggregate_executor=self._aggregate_query_with_schema,
        )
    ...
    def fetch_aggregate(
        ...
        """AggregateQuery 기반으로 문서를 조회한다."""

        return self._aggregate_query_with_schema(collection, query)
    ...
    def _aggregate_query_with_schema(
        self,
        collection: str,
        query: AggregateQuery,
        schema: Optional[CollectionSchema] = None,
        ...
        resolved_schema = self._resolve_schema(collection, schema)
        resolved_schema.validate_aggregate_query(query)
        if self._should_serialize_read_io():
            with self._engine_lock:
                return self._engine.aggregate_query(collection, query, resolved_schema)
        return self._engine.aggregate_query(collection, query, resolved_schema)
