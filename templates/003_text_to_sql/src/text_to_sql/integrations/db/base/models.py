...
class CollectionSchema(BaseModel):
    ...
    def validate_aggregate_query(self, query: "AggregateQuery") -> None:
        """집계 쿼리 입력을 스키마 기준으로 검증한다."""

        self.validate_filter_expression(query.filter_expression)

        if not query.aggregates:
            raise ValueError("집계 쿼리에는 aggregates가 최소 1개 필요합니다.")

        group_by_columns: Set[str] = set()
        for group_by in query.group_by:
            source = self.resolve_source(group_by.field, group_by.source)
            if source == FieldSource.PAYLOAD and not self.payload_field:
                raise ValueError(
                    "payload 필드가 정의되지 않아 payload group_by를 사용할 수 없습니다."
                )
            if source == FieldSource.COLUMN and group_by.field not in self.column_set():
                raise ValueError(
                    f"존재하지 않는 컬럼을 group_by에 사용할 수 없습니다: {group_by.field}"
                )
            group_by_columns.add(group_by.field)

        aggregate_aliases: Set[str] = set()
        for aggregate in query.aggregates:
            field = str(aggregate.field or "").strip()
            is_count_all = aggregate.function == AggregateFunction.COUNT and field in {
                "",
                "*",
            }
            if not is_count_all:
                if not field:
                    raise ValueError("집계 대상 컬럼이 비어 있습니다.")
                source = self.resolve_source(field, aggregate.source)
                if source == FieldSource.PAYLOAD and not self.payload_field:
                    raise ValueError(
                        "payload 필드가 정의되지 않아 payload 집계를 사용할 수 없습니다."
                    )
                if source == FieldSource.COLUMN and field not in self.column_set():
                    raise ValueError(
                        f"존재하지 않는 컬럼을 집계에 사용할 수 없습니다: {field}"
                    )
            alias = str(aggregate.alias or "").strip()
            if alias:
        ...
        for sort_field in query.sort:
            ...
            if source == FieldSource.COLUMN:
                if sort_field.field in group_by_columns:
                    continue
                if sort_field.field in aggregate_aliases:
                    continue
                if sort_field.field not in self.column_set():
                    raise ValueError(
                        f"존재하지 않는 컬럼/별칭을 정렬에 사용할 수 없습니다: {sort_field.field}"
                    )
