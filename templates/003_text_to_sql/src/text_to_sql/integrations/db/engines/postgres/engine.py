...
from text_to_sql.integrations.db.engines.postgres.condition_builder import (
    PostgresConditionBuilder,
)
from text_to_sql.integrations.db.engines.postgres.connection import (
    PostgresConnectionManager,
)
from text_to_sql.integrations.db.engines.postgres.document_mapper import (
    PostgresDocumentMapper,
)
from text_to_sql.integrations.db.engines.postgres.schema_manager import (
    PostgresSchemaManager,
)
from text_to_sql.integrations.db.engines.postgres.vector_adapter import (
    PostgresVectorAdapter,
)
from text_to_sql.integrations.db.engines.postgres.vector_store import (
    PostgresVectorStore,
)
...
class PostgresEngine(BaseDBEngine):
    ...
    def aggregate_query(
        ...
        for group_by in query.group_by:
            ...
            if source == FieldSource.PAYLOAD:
                raise ValueError(
                    "Postgres 집계 경로는 payload group_by를 지원하지 않습니다."
                )
        ...
        for aggregate in query.aggregates:
            ...
            if is_count_all:
                ...
                if source == FieldSource.PAYLOAD:
                    raise ValueError(
                        "Postgres 집계 경로는 payload 집계를 지원하지 않습니다."
                    )
        ...
        group_by_sql = ""
        if group_by_parts:
            group_by_sql = " GROUP BY " + ", ".join(group_by_parts)

        order_sql = ""
        if query.sort:
            order_by_parts = []
            for sort_field in query.sort:
                direction = "ASC" if sort_field.order == SortOrder.ASC else "DESC"
                ...
                if source == FieldSource.PAYLOAD:
                    raise ValueError(
                        "Postgres 집계 경로는 payload 정렬을 지원하지 않습니다."
                    )
                order_by_parts.append(
                    f"{self._identifier.quote_identifier(sort_key)} {direction}"
                )
        ...
        for index, row_dict in enumerate(row_dicts):
            documents.append(
                Document(
                    doc_id=index + 1,
                    fields={str(key): value for key, value in row_dict.items()},
                    payload={},
                )
            )
        return documents
