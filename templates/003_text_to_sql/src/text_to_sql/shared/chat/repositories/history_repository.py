...
디자인 패턴: 저장소 패턴
참조: src/text_to_sql/integrations/db/client.py, src/text_to_sql/shared/chat/repositories/schemas
"""
...
from text_to_sql.core.chat.models import ChatMessage, ChatRole, ChatSession, utc_now
from text_to_sql.core.chat.utils import ChatHistoryMapper
from text_to_sql.integrations.db import DBClient
from text_to_sql.integrations.db.base import (
    FieldSource,
    FilterCondition,
    FilterExpression,
    FilterOperator,
...
from text_to_sql.shared.chat.repositories.schemas import (
...
class ChatHistoryRepository:
    ...
    def list_sessions(self, limit: int, offset: int) -> list[ChatSession]:
        ...
        try:
            ...
            documents = self._client.fetch(CHAT_SESSION_COLLECTION, query)
            return [
                self._mapper.session_from_document(document) for document in documents
            ]
        except Exception as error:  # noqa: BLE001
    ...
    def append_message(
        ...
        """기존 세션에 메시지를 저장하고 세션 요약 정보를 갱신한다."""

        if not content or not content.strip():
            detail = ExceptionDetail(
                code="CHAT_MESSAGE_EMPTY", cause="content is empty"
            )
    ...
    def is_request_committed(self, request_id: str) -> bool:
        ...
        if not candidate:
            detail = ExceptionDetail(
                code="CHAT_REQUEST_ID_EMPTY", cause="request_id is empty"
            )
    ...
    def list_messages(
        ...
        try:
            ...
            return [
                self._mapper.message_from_document(document) for document in documents
            ]
        except Exception as error:  # noqa: BLE001
    ...
    def delete_session(self, session_id: str) -> tuple[bool, int]:
        ...
        try:
            ...
            deleted_message_count = (
                self._client.delete(CHAT_MESSAGE_COLLECTION)
    ...
    def _raise_repository_error(self, code: str, error: Exception) -> NoReturn:
        ...
        raise BaseAppException(
            "대화 이력 저장소 처리 중 오류가 발생했습니다.", detail, error
        ) from error
