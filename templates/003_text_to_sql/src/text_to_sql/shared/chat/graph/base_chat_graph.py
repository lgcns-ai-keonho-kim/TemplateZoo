...
class BaseChatGraph:
    ...
    def __init__(
        ...
        *,
        ...
        self._static_input: dict[str, Any] = {}
    ...
    def set_static_input(self, static_input: Mapping[str, Any] | None) -> None:
        """그래프 입력에 항상 병합할 정적 상태를 설정한다."""

        if not isinstance(static_input, Mapping):
            self._static_input = {}
            return
        self._static_input = {str(key): value for key, value in static_input.items()}
    ...
    def _build_input(
        ...
        if self._static_input:
            serialized.update(self._static_input)
        return serialized
        ...
        for node, events in raw.items():
            ...
            if not isinstance(events, Sequence):
                ...
                raise BaseAppException(
                    "stream_node 설정 형식이 올바르지 않습니다.", detail
                )
