...
디자인 패턴: 서비스 레이어
참조: src/text_to_sql/shared/chat/graph/base_chat_graph.py, src/text_to_sql/core/chat/graphs/chat_graph.py
"""
...
from text_to_sql.core.chat.const import DEFAULT_CONTEXT_WINDOW
from text_to_sql.core.chat.models import ChatMessage, ChatRole, ChatSession
from text_to_sql.shared.chat.interface import ChatServicePort, GraphPort
from text_to_sql.shared.chat.memory import ChatSessionMemoryStore
from text_to_sql.shared.chat.repositories import ChatHistoryRepository
from text_to_sql.shared.chat.runtime import clear_assistant_context
...
class ChatService(ChatServicePort):
    ...
    def list_messages(
        ...
        if session is None:
            detail = ExceptionDetail(
                code="CHAT_SESSION_NOT_FOUND", cause=f"session_id={session_id}"
            )
    ...
    def delete_session(self, session_id: str) -> None:
        with self._get_session_lock(session_id):
            ...
            try:
                clear_assistant_context(session_id=session_id)
            except Exception as error:  # noqa: BLE001 - 캐시 정리 실패가 삭제 실패로 확장되지 않도록 보호
                self._logger.warning(
                    f"chat.session.delete: assistant_context clear 실패 session_id={session_id}, error={error}"
                )
    ...
    def invoke(
        ...
        user_message = self._append_user_message_existing_session(
            session_id=session_id, message=user_query
        )
        ...
        if not content:
            detail = ExceptionDetail(
                code="CHAT_STREAM_EMPTY", cause="invoke returned empty content"
            )
    ...
    async def ainvoke(
        ...
        if not content:
            detail = ExceptionDetail(
                code="CHAT_STREAM_EMPTY", cause="ainvoke returned empty content"
            )
            raise BaseAppException("스트리밍 응답이 비어 있습니다.", detail)
        return self.append_assistant_message(session_id=session_id, content=content)
    ...
    def stream(
        ...
        if not final_content.strip():
            detail = ExceptionDetail(
                code="CHAT_STREAM_EMPTY", cause="stream returned empty content"
            )
            raise BaseAppException("스트리밍 응답이 비어 있습니다.", detail)
        yield {
            "node": done_node or self._NODE_RESPONSE,
            "event": "done",
            "data": final_content,
        }
    ...
    async def astream(
        ...
        if not final_content.strip():
            detail = ExceptionDetail(
                code="CHAT_STREAM_EMPTY", cause="astream returned empty content"
            )
            raise BaseAppException("스트리밍 응답이 비어 있습니다.", detail)
        yield {
            "node": done_node or self._NODE_RESPONSE,
    ...
    def _append_user_message_existing_session(
    ...
    def _normalize_message(self, message: str) -> str:
        ...
        if not raw.strip():
            detail = ExceptionDetail(
                code="CHAT_MESSAGE_EMPTY", cause="message is empty"
            )
