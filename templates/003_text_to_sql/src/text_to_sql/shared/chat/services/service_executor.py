...
설명: JobQueue 소비 워커와 EventBuffer 기반 SSE 중계를 관리한다.
      그래프 내부 이벤트를 공개 이벤트(start/token/references/sql_plan/sql_result/done/error)로 정규화한다.
디자인 패턴: 실행 코디네이터
참조: src/text_to_sql/shared/chat/interface/ports.py
"""
...
from text_to_sql.shared.chat.interface import ChatServicePort, ServiceExecutorPort
from text_to_sql.shared.chat.runtime import AssistantContext, AssistantContextStore
...
from text_to_sql.shared.runtime.buffer import StreamEventItem
from text_to_sql.shared.runtime.queue import QueueItem
...
class ServiceExecutor(ServiceExecutorPort):
    ...
    _NODE_BLOCKED = "blocked"
    _NODE_EXECUTOR = "executor"
    _NODE_REFERENCES = "references"
    _NODE_SQL_PLAN = "sql_plan"
    _NODE_SQL_RESULT = "sql_result"

    _EVENT_START = "start"
    _EVENT_TOKEN = "token"
    _EVENT_REFERENCES = "references"
    _EVENT_SQL_PLAN = "sql_plan"
    _EVENT_SQL_RESULT = "sql_result"
    _EVENT_DONE = "done"
    _EVENT_ERROR = "error"
    ...
    def __init__(
        ...
        assistant_context_store: AssistantContextStore | None = None,
    ) -> None:
        self._service = service
        self._job_queue = job_queue
        self._event_buffer = event_buffer
        self._assistant_context_store = assistant_context_store
    ...
    def submit_job(
        ...
        # 상태 기록은 queue.put 이전에 반영해, 초고속 워커 소비 시 COMPLETED -> QUEUED 역전이
        # 발생하지 않도록 한다.
        ...
        try:
            ...
            # put 실패 시 남은 QUEUED 표기를 정리한다.
            self._set_session_status(
                session_id=resolved_session_id, status=self._STATUS_FAILED
            )
    ...
    def _handle_job(self, job: dict[str, Any]) -> None:
        ...
        try:
            with session_lock:
                ...
                done_node = str(consumed.get("done_node") or self._NODE_RESPONSE)
        ...
        self._persist_assistant_message_sync(
            session_id=session_id,
            request_id=request_id,
    ...
    async def _consume_service_astream(
        ...
        *,
        ...
        done_node = self._NODE_RESPONSE
        done_metadata: dict[str, Any] | None = None
        latest_answer_source_meta: dict[str, Any] = {}
        ...
        async for event in self._service.astream(
            ...
            if event_type == self._EVENT_TOKEN:
                token_count += 1
            if event_type == self._EVENT_SQL_RESULT:
                candidate = self._extract_answer_source_meta(normalized.get("data"))
                if candidate:
                    latest_answer_source_meta = candidate
            if event_type == self._EVENT_DONE:
                done_emitted = True
                metadata = normalized.get("metadata")
                metadata_patch: dict[str, Any] = {"token_count": token_count}
                if latest_answer_source_meta:
                    metadata_patch["answer_source_meta"] = latest_answer_source_meta
                ...
                done_node = str(normalized.get("node") or self._NODE_RESPONSE)
    ...
    def _persist_assistant_message_sync(
        ...
        *,
        ...
        """done 이벤트 직후 assistant 메시지를 동기 저장한다."""
        ...
        max_attempt = max(0, self._persist_retry_limit)
        for attempt in range(max_attempt + 1):
            ...
            except Exception as error:  # noqa: BLE001 - 저장 실패 재시도 처리
                reason = str(error)
            ...
            if attempt >= max_attempt:
                ...
                return
            self._service_logger.warning(
                f"chat.persist.retry: session_id={session_id}, request_id={request_id}, attempt={attempt + 1}, reason={reason}"
            )
    ...
    def _process_persist_task(self, task: dict[str, Any]) -> None:
        ...
        node = str(task.get("node") or self._NODE_RESPONSE)
    ...
    def _cache_assistant_context(
        ...
        *,
        ...
        """직전 assistant 응답을 세션 캐시에 저장한다."""

        store = self._assistant_context_store
        if store is None:
            return
        ...
        answer_source_meta = self._extract_answer_source_meta(metadata)
        ...
        try:
            store.set(context)
        except Exception as error:  # noqa: BLE001 - 캐시 실패가 스트림 자체를 중단시키면 안 된다.
            self._service_logger.warning(
                f"assistant.context.cache_failed: session_id={normalized_session_id}, request_id={normalized_request_id}, error={error}"
            )

    def _extract_answer_source_meta(self, value: Any) -> dict[str, Any]:
        if isinstance(value, dict):
            raw_meta = value.get("answer_source_meta")
            if isinstance(raw_meta, dict):
                return {str(key): meta_value for key, meta_value in raw_meta.items()}
        return {}
    ...
    def _normalize_graph_event(self, event: Any) -> dict[str, Any] | None:
        ...
        if event_name == "token":
            ...
            return {
                ...
                "node": node or self._NODE_RESPONSE,
                "data": content,
                "metadata": metadata,
            }

        if event_name == "assistant_message":
            if node != self._NODE_BLOCKED:
                return None
            content = str(data or "")
            if not content:
                return None
        ...
        if event_name == "references":
            ...
            # node가 비어 있는 경우 references를 기본 노드명으로 사용한다.
            return {
                "event": self._EVENT_REFERENCES,
                "node": node or self._NODE_REFERENCES,
                "data": payload,
                "metadata": metadata,
            }

        if event_name == "sql_plan":
            return {
                "event": self._EVENT_SQL_PLAN,
                "node": node or self._NODE_SQL_PLAN,
                "data": data,
                "metadata": metadata,
            }

        if event_name == "sql_result":
            return {
                "event": self._EVENT_SQL_RESULT,
                "node": node or self._NODE_SQL_RESULT,
                "data": data,
                "metadata": metadata,
            }
        ...
        if event_name == "error":
            return {
                ...
                "node": node or self._NODE_EXECUTOR,
                "data": str(data or ""),
                "metadata": metadata,
            }
