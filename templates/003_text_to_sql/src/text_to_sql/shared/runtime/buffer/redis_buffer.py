...
디자인 패턴: 어댑터 패턴
참조: src/text_to_sql/shared/runtime/buffer/model.py
"""
...
from text_to_sql.shared.runtime.buffer.model import EventBufferConfig, StreamEventItem
...
class RedisEventBuffer:
    ...
    def push(
        self, session_id: str, request_id: str, event: dict[str, Any] | StreamEventItem
    ) -> StreamEventItem:
        """요청 단위 키에 이벤트를 적재한다."""
    ...
    def pop(
        self, session_id: str, request_id: str, timeout: float | None = None
    ) -> StreamEventItem | None:
    ...
    def _blocking_pop(self, key: str, timeout: float | None):
        ...
        value_bytes = (
            value_raw if isinstance(value_raw, bytes) else str(value_raw).encode()
        )
        return key_bytes, value_bytes

    def _to_item(
        self, request_id: str, event: dict[str, Any] | StreamEventItem
    ) -> StreamEventItem:
        if isinstance(event, StreamEventItem):
            if event.request_id != request_id:
                raise ValueError(
                    "event.request_id와 요청 request_id가 일치하지 않습니다."
                )
            return event
