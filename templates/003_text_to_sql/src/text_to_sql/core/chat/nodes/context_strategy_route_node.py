...
디자인 패턴: 모듈 조립
참조: src/text_to_sql/core/chat/nodes/context_strategy_node.py, src/text_to_sql/shared/chat/nodes/branch_node.py
"""
...
_context_strategy_route_logger: Logger = create_default_logger(
    "ContextStrategyRouteNode"
)

# NOTE:
# - selector_key: LLM 분류 원시 출력(state["context_strategy_raw"])을 읽는다.
# - branch_map: 정규화된 selector를 최종 전략 토큰으로 매핑한다.
# - default_branch/fallback_selector: 허용값이 아니면 보수적으로 USE_SQL을 사용한다.
context_strategy_route_node = BranchNode(
    ...
    branch_map={
        ...
        CONTEXT_STRATEGY_USE_METADATA: CONTEXT_STRATEGY_USE_METADATA,
        CONTEXT_STRATEGY_USE_SQL: CONTEXT_STRATEGY_USE_SQL,
    },
    default_branch=CONTEXT_STRATEGY_USE_SQL,
    ...
    fallback_selector=CONTEXT_STRATEGY_USE_SQL,
