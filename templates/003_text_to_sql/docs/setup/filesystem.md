...
이 문서는 `src/text_to_sql/integrations/fs`를 이용해 로그를 파일로 저장하는 방법과,
...
| 경로 | 역할 |
| --- | --- |
| `src/text_to_sql/integrations/fs/base/engine.py` | 파일 엔진 인터페이스 |
| `src/text_to_sql/integrations/fs/engines/local.py` | 로컬 파일 시스템 구현 |
| `src/text_to_sql/integrations/fs/file_repository.py` | 로그 파일 저장소 |
| `src/text_to_sql/shared/logging/logger.py` | Logger/LogRepository 인터페이스 |
| `src/text_to_sql/integrations/llm/client.py` | 로깅 저장소를 주입해 LLM 호출 로그 저장 가능 |
...
```python
from text_to_sql.integrations.fs import FileLogRepository
from text_to_sql.shared.logging import InMemoryLogger
...
```python
from text_to_sql.integrations.fs.base import BaseFSEngine
