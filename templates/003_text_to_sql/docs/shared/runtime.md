...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 런타임 공개 API | `src/text_to_sql/shared/runtime/__init__.py` | Queue/Buffer/Worker/ThreadPool 공개 |
| Queue 모델 | `src/text_to_sql/shared/runtime/queue/model.py` | `QueueConfig`, `QueueItem` |
| InMemory Queue | `src/text_to_sql/shared/runtime/queue/in_memory_queue.py` | 프로세스 내부 큐 |
| Redis Queue | `src/text_to_sql/shared/runtime/queue/redis_queue.py` | Redis 리스트 기반 큐 |
| Buffer 모델 | `src/text_to_sql/shared/runtime/buffer/model.py` | `EventBufferConfig`, `StreamEventItem` |
| InMemory Buffer | `src/text_to_sql/shared/runtime/buffer/in_memory_buffer.py` | 요청 버킷 기반 이벤트 버퍼 |
| Redis Buffer | `src/text_to_sql/shared/runtime/buffer/redis_buffer.py` | Redis 리스트 기반 이벤트 버퍼 |
| Worker 모델 | `src/text_to_sql/shared/runtime/worker/model.py` | `WorkerConfig`, `WorkerState` |
| Worker 구현 | `src/text_to_sql/shared/runtime/worker/worker.py` | 큐 소비 워커 |
| ThreadPool 모델 | `src/text_to_sql/shared/runtime/thread_pool/model.py` | `ThreadPoolConfig`, `TaskRecord` |
| ThreadPool 구현 | `src/text_to_sql/shared/runtime/thread_pool/thread_pool.py` | 태스크 제출/종료 |

연동 스크립트:

1. `src/text_to_sql/api/chat/services/runtime.py`
2. `src/text_to_sql/shared/chat/services/service_executor.py`
...
| Redis 연결 실패 | 호스트/포트/DB/PW 설정 오류 또는 패키지 미설치 | `queue/redis_queue.py`, `buffer/redis_buffer.py` | Redis 환경 변수/의존성 확인 |
