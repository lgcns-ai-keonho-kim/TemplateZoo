# SqliteConnectionManager 가이드

이 문서는 `src/rag_chatbot/integrations/db/engines/sqlite/connection.py`의 현재 구현을 기준으로 역할과 유지보수 포인트를 정리한다.
...
- 클래스 `SqliteConnectionManager`
  공개 메서드: `supports_vector_search`, `connect`, `close`, `ensure_connection`
