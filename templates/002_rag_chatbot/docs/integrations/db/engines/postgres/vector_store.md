# PostgresVectorStore 가이드

이 문서는 `src/rag_chatbot/integrations/db/engines/postgres/vector_store.py`의 현재 구현을 기준으로 역할과 유지보수 포인트를 정리한다.
...
- 클래스 `PostgresVectorStore`
  공개 메서드: `adapter`, `has_vector_column`, `ensure_vector_extension`, `ensure_vector_index`, `drop_vector_index_if_needed`
