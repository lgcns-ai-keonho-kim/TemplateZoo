# RedisConnectionManager 가이드

이 문서는 `src/rag_chatbot/integrations/db/engines/redis/connection.py`의 현재 구현을 기준으로 역할과 유지보수 포인트를 정리한다.
...
- 클래스 `RedisConnectionManager`
  공개 메서드: `connect`, `close`, `ensure_client`
