# LanceFilterEngine 가이드

이 문서는 `src/rag_chatbot/integrations/db/engines/lancedb/filter_engine.py`의 현재 구현을 기준으로 역할과 유지보수 포인트를 정리한다.
...
- 클래스 `LanceFilterEngine`
  공개 메서드: `build_where_clause`, `build_eq_clause`, `distance_to_similarity`, `apply_sort`, `match_filter`
