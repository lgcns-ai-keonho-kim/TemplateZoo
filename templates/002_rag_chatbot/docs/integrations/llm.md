...
이 문서는 `src/rag_chatbot/integrations/llm/client.py`의 `LLMClient` 인터페이스와 동작 규칙을 코드 기준으로 정리한다.
...
| 파일 | 역할 |
| --- | --- |
| `src/rag_chatbot/integrations/llm/client.py` | LLMClient 구현 |
| `src/rag_chatbot/integrations/llm/__init__.py` | 공개 API 제공 |
| `src/rag_chatbot/shared/logging/*` | 로깅 모델/저장소/로거 |
| `src/rag_chatbot/shared/exceptions/*` | 공통 예외 모델 |
| `src/rag_chatbot/core/chat/nodes/response_node.py` | response 노드의 LLMClient 사용 예 |
| `src/rag_chatbot/core/chat/nodes/safeguard_node.py` | safeguard 노드의 LLMClient 사용 예 |
...
4. 문서 경로가 실제 `src/rag_chatbot/integrations/llm` 구조와 일치하는가
