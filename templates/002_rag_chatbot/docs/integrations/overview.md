...
| 용어 | 의미 | 관련 스크립트 |
| --- | --- | --- |
| 통합 계층 | 외부 시스템(DB, LLM, 파일시스템)과 직접 통신하는 계층 | `src/rag_chatbot/integrations/*` |
...
| `src/rag_chatbot/integrations/llm` | LangChain 모델 래핑과 로깅/예외 표준화 | `client.py` |
| `src/rag_chatbot/integrations/fs` | 파일 시스템 추상화와 파일 로그 저장소 | `base/engine.py`, `engines/local.py`, `file_repository.py` |
| `src/rag_chatbot/integrations/__init__.py` | integrations 공개 API | `__init__.py` |
...
3. 엔진 교체는 조립 지점(`src/rag_chatbot/api/chat/services/runtime.py`)에서 수행한다.
