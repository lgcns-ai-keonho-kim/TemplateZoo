...
| RAG 노드 | 검색/필터로 참고 문맥을 생성하는 단계 | `rag_node` |
...
| 턴 결과 | `src/rag_chatbot/core/chat/models/turn_result.py` | 1턴 처리 결과 묶음 모델 |
| 상수 | `src/rag_chatbot/core/chat/const/settings.py` | DB 경로, 페이지, 문맥 길이 기본값 |
| 차단 문구 | `src/rag_chatbot/core/chat/const/messages/safeguard.py` | 차단 메시지 Enum |
...
| 응답 노드 | `src/rag_chatbot/core/chat/nodes/response_node.py` | 일반 응답 생성 |
| RAG 노드 | `src/rag_chatbot/shared/chat/nodes/rag_node.py` | RAG 문맥/레퍼런스 생성 |
| 분류 노드 | `src/rag_chatbot/core/chat/nodes/safeguard_node.py` | PASS/차단 라벨 분류 |
| 분기 노드 | `src/rag_chatbot/core/chat/nodes/safeguard_route_node.py` | response/blocked 분기 |
| 차단 노드 | `src/rag_chatbot/core/chat/nodes/safeguard_message_node.py` | 차단 문구 생성 |
| 프롬프트 | `src/rag_chatbot/core/chat/prompts/chat_prompt.py`, `src/rag_chatbot/core/chat/prompts/safeguard_prompt.py` | 시스템 프롬프트 |
| 매퍼 | `src/rag_chatbot/core/chat/utils/mapper.py` | 도메인 모델과 DB 문서 변환 |

연동 스크립트:

1. `src/rag_chatbot/shared/chat/graph/base_chat_graph.py`
2. `src/rag_chatbot/shared/chat/nodes/llm_node.py`
3. `src/rag_chatbot/shared/chat/nodes/branch_node.py`
4. `src/rag_chatbot/shared/chat/nodes/message_node.py`
...
`src/rag_chatbot/core/chat/state/graph_state.py` 기준:
...
- 파일: `src/rag_chatbot/core/chat/nodes/safeguard_route_node.py`
...
- 파일: `src/rag_chatbot/core/chat/nodes/response_node.py`
- 내부 구현: `LLMNode`
...
## 5-5. safeguard_message_node

- 파일: `src/rag_chatbot/core/chat/nodes/safeguard_message_node.py`
- 내부 구현: `MessageNode`
...
`src/rag_chatbot/core/chat/const/settings.py` 기준:
...
`src/rag_chatbot/shared/chat/services/chat_service.py`에서 core 그래프를 호출한다.
...
`src/rag_chatbot/shared/chat/services/service_executor.py`는 core 이벤트를 API 이벤트로 변환한다.
