"""
목적: Elasticsearch ingestion 실행 스크립트를 제공한다.
설명: data/ingestion-doc 문서를 청킹/임베딩/Elasticsearch 업서트 순서로 처리한다.
디자인 패턴: 스크립트 오케스트레이션
...
# `python ingestion/ingest_elasticsearch.py` 실행 시 프로젝트 루트를 import 경로에 보정한다.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Elasticsearch ingestion 실행")
    parser.add_argument(
        "--input-root",
        type=Path,
...
def main() -> None:
    ...
    print(
        f"[진행][ingest][elasticsearch] 시작: input_root={args.input_root}, "
    ...
    print(f"[진행][ingest][elasticsearch] 청킹 완료: count={len(chunks)}")

    chunks = run_embedding_step(chunks, embedder=embedder)
    print("[진행][ingest][elasticsearch] 임베딩 생성 완료")

    run_upsert_elasticsearch_step(chunks, embedder=embedder)
    print("[진행][ingest][elasticsearch] 업서트 완료")
