...
디자인 패턴: 저장소 패턴
참조: src/rag_chatbot/integrations/embedding/client.py, src/rag_chatbot/shared/logging/models.py
"""
...
from rag_chatbot.shared.logging.logger import LogRepository
from rag_chatbot.shared.logging.models import LogContext, LogRecord

if TYPE_CHECKING:
    from rag_chatbot.integrations.db import DBClient
    from rag_chatbot.integrations.db.base import CollectionSchema, Document
...
class EmbeddingLogRepository(LogRepository):
    ...
    def list(self) -> List[LogRecord]:
        ...
        from rag_chatbot.integrations.db.base import Query
    ...
    def _ensure_db_available(self) -> None:
        try:
            from rag_chatbot.integrations.db import DBClient  # noqa: F401
        except ImportError as error:
            raise RuntimeError("DB 통합 모듈을 불러올 수 없습니다.") from error
