...
디자인 패턴: 프록시, 데코레이터
참조: src/rag_chatbot/shared/logging, src/rag_chatbot/shared/exceptions
"""
...
class EmbeddingClient(Embeddings):
        ...
        *,
        ...
        if expected_count <= 0:
            if raw not in (None, []):
                ...
                raise BaseAppException("문서 임베딩 결과 개수가 입력 개수와 다릅니다.", detail)
            return []
        ...
        vectors = [self._coerce_vector(vector, index=index) for index, vector in enumerate(raw_vectors)]
    ...
    def _coerce_vector(self, value: Any, *, index: int) -> list[float]:
        try:
            ...
            raise BaseAppException("임베딩 벡터 형식이 올바르지 않습니다.", detail) from None
    ...
    def _build_call_error(self, action: str, error: Exception) -> BaseAppException:
        ...
        return BaseAppException(message_map.get(action, "임베딩 호출에 실패했습니다."), detail, error)
    ...
    def _log_start(
        ...
        *,
        ...
        metadata.update({"input_count": int(input_count), "input_chars": int(input_chars)})
