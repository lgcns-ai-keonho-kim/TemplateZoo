...
설명: get_tools/list_specs/list_for_selector 반환 형태와 planner 호환 alias 동작을 확인한다.
디자인 패턴: 단위 테스트
참조: src/single_request_tool_agent/shared/agent/tools/registry.py
"""

from __future__ import annotations

from single_request_tool_agent.shared.agent.tools.registry import ToolRegistry
...
def test_list_for_selector_uses_registered_specs() -> None:
    """Selector 노출 스펙은 등록된 Tool 정보와 동일해야 한다."""
    ...
    selector_specs = registry.list_for_selector()
    assert len(selector_specs) == 1
    assert selector_specs[0]["name"] == "sum_tool"
    assert selector_specs[0]["description"] == "two numbers sum"
    assert selector_specs[0]["args_schema"]["required"] == ["a", "b"]


def test_list_for_planner_keeps_backward_compatible_alias() -> None:
    """기존 planner alias도 같은 스펙을 반환해야 한다."""
