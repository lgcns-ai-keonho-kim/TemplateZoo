...
LLM 기반 1회성 Agent 애플리케이션을 빠르게 시작하기 위한 Python/FastAPI 템플릿이다.
...
```bash
cp src/single_request_tool_agent/resources/dev/.env.sample src/single_request_tool_agent/resources/dev/.env
```
...
3. 값이 비어 있으면 `local`
4. 필요 시 `src/single_request_tool_agent/resources/<env>/.env`
...
4. `core/agent/graphs/chat_graph.py`가 Tool 선택, 실행, 재시도, 응답 생성을 담당하는 그래프를 준비한다.
...
### 4-3. 기본 `/agent` 실행에 직접 필요하지 않은 키

아래 키는 `.env.sample`에 포함되어 있지만, 기본 `/agent` 실행 경로가 직접 사용하지는 않는다.

| 범주 | 예시 키 | 사용 맥락 |
| --- | --- | --- |
| 저장/메모리 | `CHAT_DB_PATH`, `CHAT_MEMORY_MAX_MESSAGES` | 세션 저장소 기반 조립이나 메모리 보관 정책을 별도로 검토할 때 사용 |
...
### 5-2. `src/single_request_tool_agent/` 디렉터리 레벨 책임 맵

| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/single_request_tool_agent/api` | FastAPI 진입 계층 | 라우터 등록, 요청/응답 모델, 런타임 의존성 주입 |
| `src/single_request_tool_agent/core` | Agent 도메인 계층 | 그래프 상태, 프롬프트, 노드 조립, Tool 정의 |
| `src/single_request_tool_agent/shared` | 공용 실행 계층 | `AgentService`, Tool 레지스트리, 공통 예외/로깅/설정, 범용 런타임 유틸 |
| `src/single_request_tool_agent/integrations` | 외부 연동 계층 | LLM/DB/파일 시스템/임베딩 어댑터 |
| `src/single_request_tool_agent/resources` | 환경 리소스 계층 | `dev/stg/prod`별 `.env.sample` 및 환경 파일 보관 |
| `src/single_request_tool_agent/static` | 웹 UI 정적 리소스 계층 | HTML/CSS/JS와 화면 상태 처리 |

### 5-3. `src/single_request_tool_agent/api` 하위 구조

```text
src/single_request_tool_agent/api/
...
### 5-4. `src/single_request_tool_agent/core/agent` 하위 구조

```text
src/single_request_tool_agent/core/agent/
  const/                # 기본 메시지와 설정 상수
  models/               # 실행 결과, 엔티티 모델
  state/                # 그래프 상태 계약
...
### 5-5. `src/single_request_tool_agent/shared` 하위 구조

```text
src/single_request_tool_agent/shared/
