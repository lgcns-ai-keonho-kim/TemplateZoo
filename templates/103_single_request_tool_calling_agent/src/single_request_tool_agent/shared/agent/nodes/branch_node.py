...
디자인 패턴: 규칙 기반 라우터
참조: src/single_request_tool_agent/core/agent/nodes/safeguard_route_node.py
"""
...
class BranchNode:
    ...
    def _run(self, state: Mapping[str, Any]) -> dict[str, str]:
        ...
        if (
            self._allowed_selectors is not None
            and normalized not in self._allowed_selectors
        ):
    ...
    def _normalize_collection(self, values: Collection[str] | None) -> set[str] | None:
        ...
        if values is None:
            return None
        return {
            self._normalize_value(str(item)) for item in values if str(item).strip()
        }
