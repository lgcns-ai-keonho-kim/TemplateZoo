...
디자인 패턴: 저장소 패턴
참조: src/single_request_tool_agent/shared/agent/services/chat_service.py, src/single_request_tool_agent/core/agent/models/entities.py
"""
...
class ChatSessionMemoryStore:
    ...
    def ensure_session(
        ...
        self._logger.debug(
            f"세션 메모리 초기화: session_id={session_id}, count={len(loaded)}"
        )
    ...
    def lrange(self, session_id: str, start: int, end: int) -> list[ChatMessage]:
        ...
        if normalized_start > normalized_end:
            return []
        return [
            self._copy_message(item)
            for item in source[normalized_start : normalized_end + 1]
        ]
