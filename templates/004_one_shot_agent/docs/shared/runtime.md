...
`src/one_shot_agent/shared/runtime`의 보존 유틸을 코드 기준으로 정리한다.
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 런타임 공개 API | `src/one_shot_agent/shared/runtime/__init__.py` | Queue/Worker/ThreadPool 공개 |
| Queue 모델 | `src/one_shot_agent/shared/runtime/queue/model.py` | `QueueConfig`, `QueueItem` |
| InMemory Queue | `src/one_shot_agent/shared/runtime/queue/in_memory_queue.py` | 프로세스 내부 큐 |
| Redis Queue | `src/one_shot_agent/shared/runtime/queue/redis_queue.py` | Redis 리스트 기반 큐 |
| Worker 모델 | `src/one_shot_agent/shared/runtime/worker/model.py` | `WorkerConfig`, `WorkerState` |
| Worker 구현 | `src/one_shot_agent/shared/runtime/worker/worker.py` | 큐 소비 워커 |
| ThreadPool 모델 | `src/one_shot_agent/shared/runtime/thread_pool/model.py` | `ThreadPoolConfig`, `TaskRecord` |
| ThreadPool 구현 | `src/one_shot_agent/shared/runtime/thread_pool/thread_pool.py` | 태스크 제출/종료 |
