...
`src/one_shot_agent/integrations/fs`를 이용해 로그를 파일로 저장하는 방법과,
...
| 경로 | 역할 |
| --- | --- |
| `src/one_shot_agent/integrations/fs/base/engine.py` | 파일 엔진 인터페이스 |
| `src/one_shot_agent/integrations/fs/engines/local.py` | 로컬 파일 시스템 구현 |
| `src/one_shot_agent/integrations/fs/file_repository.py` | 로그 파일 저장소 |
| `src/one_shot_agent/shared/logging/logger.py` | 로깅 공개 API 파사드 |
| `src/one_shot_agent/shared/logging/_logger_base.py` | Logger 추상 베이스 |
| `src/one_shot_agent/shared/logging/_log_repository_interface.py` | LogRepository 인터페이스 원본 |
| `src/one_shot_agent/integrations/llm/client.py` | 로깅 저장소를 주입해 LLM 호출 로그 저장 가능 |
...
```python
from one_shot_agent.integrations.fs.base import BaseFSEngine
