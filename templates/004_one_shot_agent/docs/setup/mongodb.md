...
MongoDB 설치, 인증 설정, `.env` 구성, 그리고 이 프로젝트의 저장소 조립에 연결하는 방법을 정리한다.
...
| 경로 | 역할 |
| --- | --- |
| `src/one_shot_agent/integrations/db/engines/mongodb/engine.py` | MongoDB 엔진 본체 |
| `src/one_shot_agent/integrations/db/engines/mongodb/connection.py` | 연결/DB 객체 초기화 |
| `src/one_shot_agent/integrations/db/engines/mongodb/schema_manager.py` | 컬렉션 생성/필드 변경 |
| `src/one_shot_agent/integrations/db/engines/mongodb/filter_builder.py` | Query -> Mongo 필터 변환 |
| `src/one_shot_agent/api/agent/services/runtime.py` | 저장소 조립 지점 |
| `src/one_shot_agent/shared/agent/repositories/history_repository.py` | `db_client` 주입형 저장소 |
...
from one_shot_agent.integrations.db.engines.mongodb import MongoDBEngine
from one_shot_agent.shared.agent import ChatHistoryRepository
