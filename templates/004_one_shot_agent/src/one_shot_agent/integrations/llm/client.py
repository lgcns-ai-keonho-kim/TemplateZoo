...
디자인 패턴: 프록시, 데코레이터
참조: src/one_shot_agent/shared/logging, src/one_shot_agent/shared/exceptions
"""
...
from one_shot_agent.integrations.llm._client_mixin import _LLMClientMixin
from one_shot_agent.shared.exceptions import (
    BaseAppException,
    ExceptionDetail,
)
from one_shot_agent.shared.logging import (
    InMemoryLogger,
    LogContext,
    LogLevel,
    LogRepository,
...
if TYPE_CHECKING:
    from one_shot_agent.integrations.db import DBClient
    from one_shot_agent.integrations.db.base import (
        BaseDBEngine,
        CollectionSchema,
    )
...
class LLMClient(_LLMClientMixin, BaseChatModel):
    ...
    def chat(self, messages: Sequence[BaseMessage], **kwargs: Any) -> BaseMessage:
        ...
        if not content:
            detail = ExceptionDetail(
                code="LLM_STREAM_EMPTY", cause="chat stream produced empty content"
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise BaseAppException(
                "LLM 비동기 호출에 실패했습니다.", detail, error
            ) from error
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise BaseAppException(
                "LLM 스트리밍 호출에 실패했습니다.", detail, error
            ) from error
        finally:
            if completed:
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        completed = False
        try:
            async for chunk in self._aiter_stream_chunks(
            ...
            raise BaseAppException(
                "LLM 비동기 스트리밍 호출에 실패했습니다.", detail, error
            ) from error
        finally:
            if completed:
    ...
    def _resolve_logging(
        ...
        from one_shot_agent.integrations.db import DBClient as _DBClient
        from one_shot_agent.integrations.db.base import (
    ...
    def _build_llm_repository(
        ...
        from one_shot_agent.shared.logging.llm_repository import (
            LLMLogRepository,
        )
    ...
    def _log_start(
        ...
        self._safe_log(
            LogLevel.INFO, f"LLM {action} 호출 시작", context=context, metadata=metadata
        )

    def _log_success(
        self, action: str, start: float, result: Optional[ChatResult]
    ) -> None:
        ...
        context = self._get_context()
        self._safe_log(
            LogLevel.INFO, f"LLM {action} 호출 성공", context=context, metadata=metadata
        )
