...
`src/single_request_agent/shared/config`의 설정 병합과 런타임 환경 로딩 규칙을 코드 기준으로 정리한다.
...
| 파일 | 역할 |
| --- | --- |
| `src/single_request_agent/shared/config/loader.py` | 설정 소스 추가와 병합 수행 |
| `src/single_request_agent/shared/config/runtime_env_loader.py` | 실행 환경 판별과 `.env` 로딩 |
| `src/single_request_agent/shared/config/__init__.py` | 공개 API 제공 |
| `src/single_request_agent/shared/const/__init__.py` | 인코딩/구분자 상수 제공 |
| `src/single_request_agent/api/main.py` | 런타임 환경 로더 호출 지점 |
...
4. `dev/stg/prod`면 `src/single_request_agent/resources/<env>/.env`를 추가 로드한다.
...
1. `src/single_request_agent/api/main.py`에서 `RuntimeEnvironmentLoader().load()` 실행
