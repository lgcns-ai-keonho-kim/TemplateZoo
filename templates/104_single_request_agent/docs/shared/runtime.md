...
`src/single_request_agent/shared/runtime`의 Queue, EventBuffer, Worker, ThreadPool 구성요소를 코드 기준으로 정리한다.
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 런타임 공개 API | `src/single_request_agent/shared/runtime/__init__.py` | Queue/Buffer/Worker/ThreadPool 공개 |
...
| Buffer 모델 | `src/single_request_agent/shared/runtime/buffer/model.py` | `EventBufferConfig`, `StreamEventItem` |
| InMemory Buffer | `src/single_request_agent/shared/runtime/buffer/in_memory_buffer.py` | 요청 버킷 기반 이벤트 버퍼 |
| Redis Buffer | `src/single_request_agent/shared/runtime/buffer/redis_buffer.py` | Redis 리스트 기반 이벤트 버퍼 |
...
1. `src/single_request_agent/api/agent/services/runtime.py`
2. `src/single_request_agent/shared/agent/services/service_executor.py`
