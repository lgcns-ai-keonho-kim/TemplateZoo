# Tool Selector + Retry Agent Template

LLM 기반 Tool Selector Agent 애플리케이션을 빠르게 시작하기 위한 Python/FastAPI 템플릿이다.
...
Tool selector는 `ToolRegistry`에 등록된 Tool 스펙을 기준으로 `tool_calls[]` JSON을 만들고, Execute 단계는 선택된 Tool만 fan-out으로 호출한다.
...
```bash
cp src/tool_proxy_agent/resources/dev/.env.sample src/tool_proxy_agent/resources/dev/.env
```
...
3. 값이 비어 있으면 `local`
4. 필요 시 `src/tool_proxy_agent/resources/<env>/.env`

### 1-4. 서버 실행

```bash
uv run uvicorn tool_proxy_agent.api.main:app --host 0.0.0.0 --port 8000 --reload
```
...
| `DELETE` | `/ui-api/chat/sessions` | `200` | UI 세션 전체 삭제 |
...
- `add_number`는 실제 덧셈 Tool 예시이며 Tool selector가 정수 계산 질문에 사용할 수 있다.
...
flowchart TD
    ...
    TEP --> TEF[tool_execute_fanout_route]
    TEF -->|fanout send tool_exec| TEX[tool_exec]
    TEF -->|default_branch| TEC[tool_execute_collect]
    TEX --> TEC
    TEC --> RR[retry_route]
    ...
    RPP --> RLLM[retry_llm]
    RLLM --> RPARSE[retry_parse]
    RPARSE --> RV[retry_validate]
    RV --> TER
...
| `safeguard_route` | 결과값 정규화/별칭 교정 후 분기 결정 | `safeguard_result` | `safeguard_route`, `safeguard_result` | `tool_selector_prepare` 또는 `blocked` |
...
2. 필수 식별자: `session_id`, `request_id`, `tool_call_id(tool_* 이벤트)`
...
```text
.
  src/tool_proxy_agent/
    api/                # HTTP 진입점, DTO, DI 조립
    core/               # tool selector + retry 도메인 규칙과 그래프 조립
...
### 6-2. `src/tool_proxy_agent/` 디렉터리 레벨 책임 맵

| 경로 | 역할 | 구현 책임 범위 | 책임 밖 범위 |
| --- | --- | --- | --- |
| `src/tool_proxy_agent/api` | FastAPI 진입 계층 | 라우터 등록, 요청/응답 모델, HTTP 예외 변환, 런타임 의존성 주입 | 도메인 규칙 자체, Queue/DB 엔진 세부 구현 |
| `src/tool_proxy_agent/core` | Agent 도메인 계층 | 상태 모델, 프롬프트, 노드 조립, 그래프 분기 규칙 | HTTP 프로토콜, 저장소/큐 인프라 운영 |
| `src/tool_proxy_agent/shared` | 공용 실행 계층 | 서비스 오케스트레이션, 저장소, 메모리, 런타임 유틸리티, 공통 예외/로깅/설정 | 특정 외부 제품 API 세부 구현 |
| `src/tool_proxy_agent/integrations` | 외부 연동 계층 | LLM/DB/파일 시스템/임베딩 어댑터, 연결/매퍼/엔진 | Agent 유스케이스 정책, API 라우팅 |
| `src/tool_proxy_agent/resources` | 환경 리소스 계층 | `dev/stg/prod`별 `.env.sample` 및 환경 파일 보관 | 런타임 로직 |
| `src/tool_proxy_agent/static` | 웹 UI 정적 리소스 계층 | HTML/CSS/JS, 아이콘, 프런트 상태 처리 | FastAPI 비즈니스 로직, 도메인 정책 |

### 6-3. `src/tool_proxy_agent/api` 하위 구조

```text
src/tool_proxy_agent/api/
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/tool_proxy_agent/api/chat/models` | Chat DTO | `/chat` 요청/응답 검증, SSE 구독 입력/출력 모델 |
| `src/tool_proxy_agent/api/chat/routers` | Chat 라우터 | 작업 제출, 스트림 구독, 세션 스냅샷 조회 |
| `src/tool_proxy_agent/api/chat/services` | Chat 런타임 조립 | `ChatService`, `ServiceExecutor`, 큐/버퍼 싱글턴 조립 |
| `src/tool_proxy_agent/api/chat/utils` | Chat API 보조 유틸 | 도메인 모델을 API 응답 형태로 변환 |
| `src/tool_proxy_agent/api/ui/models` | UI DTO | 세션/메시지 목록 응답 모델 |
| `src/tool_proxy_agent/api/ui/routers` | UI 라우터 | 세션 생성, 조회, 전체 삭제, 단건 삭제 엔드포인트 |
| `src/tool_proxy_agent/api/ui/services` | UI 서비스 | `ChatService` 결과를 UI 응답 모델로 매핑 |
| `src/tool_proxy_agent/api/ui/utils` | UI 매퍼 | 도메인 엔티티를 UI DTO로 변환 |
| `src/tool_proxy_agent/api/health` | Health API | `/health` 단일 엔드포인트와 응답 모델 관리 |

### 6-4. `src/tool_proxy_agent/core/chat` 하위 구조

```text
src/tool_proxy_agent/core/chat/
  const/                # 도메인 상수와 기본 메시지
  models/               # ChatSession, ChatMessage 같은 엔티티
  state/                # 그래프 상태 키 정의
  prompts/              # 시스템 프롬프트
  nodes/                # safeguard/tool selector/execute/retry/response 노드 조립체
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/tool_proxy_agent/core/chat/const` | 도메인 설정값 | 컬렉션명, 페이지 크기, 기본 문맥 길이, 차단 메시지 |
| `src/tool_proxy_agent/core/chat/models` | 핵심 엔티티 | 세션, 메시지, 역할 모델 정의 |
| `src/tool_proxy_agent/core/chat/state` | 그래프 상태 계약 | 노드 간 전달되는 상태 키의 타입 계약 |
| `src/tool_proxy_agent/core/chat/prompts` | 프롬프트 정책 | safeguard, tool selector, retry, response 프롬프트 |
| `src/tool_proxy_agent/core/chat/nodes` | 도메인 노드 조립 | 실제 사용할 LLM/분기/실행/메시지 노드 인스턴스 선언 |
| `src/tool_proxy_agent/core/chat/graphs` | 그래프 정의 | 노드 연결, 진입점, 조건 분기, stream 정책 설정 |
| `src/tool_proxy_agent/core/chat/tools` | Tool 구현과 등록 | 기본 Tool 함수와 `ToolRegistry` singleton 구성 |
| `src/tool_proxy_agent/core/chat/utils` | 도메인 매핑 유틸 | 도메인 모델과 문서/DB 표현 사이 매핑 지원 |

### 6-5. `src/tool_proxy_agent/shared` 하위 구조

```text
src/tool_proxy_agent/shared/
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/tool_proxy_agent/shared/chat/interface` | 포트 계약 | 그래프, 서비스, 실행기 인터페이스 정의 |
| `src/tool_proxy_agent/shared/chat/graph` | 그래프 공통 실행기 | 그래프 컴파일, 이벤트 표준화, stream 필터링 |
| `src/tool_proxy_agent/shared/chat/nodes` | 재사용 노드 | `LLMNode`, `BranchNode`, `MessageNode`, `ToolExecNode` 같은 범용 노드 |
| `src/tool_proxy_agent/shared/chat/repositories` | 대화 이력 저장소 | 세션/메시지 CRUD, `request_id` 멱등 저장 관리 |
| `src/tool_proxy_agent/shared/chat/memory` | 세션 메모리 캐시 | 최근 메시지 컨텍스트 메모리 유지 |
| `src/tool_proxy_agent/shared/chat/services` | 서비스 계층 | `ChatService`, `ServiceExecutor` 실행 오케스트레이션 |
| `src/tool_proxy_agent/shared/chat/tools` | Tool 공통 타입 | Tool 계약, registry, selector payload 유틸 |
| `src/tool_proxy_agent/shared/runtime` | 실행 인프라 | Queue, EventBuffer, Worker, ThreadPool 제공 |
| `src/tool_proxy_agent/shared/config` | 설정 로딩 | `.env`/JSON/환경 변수 병합 및 런타임 환경 해석 |
| `src/tool_proxy_agent/shared/logging` | 공통 로깅 | Logger, 로그 모델, 저장소 구현 |
| `src/tool_proxy_agent/shared/exceptions` | 공통 예외 | 코드/원인 추적이 가능한 애플리케이션 예외 정의 |

### 6-6. `src/tool_proxy_agent/integrations` 하위 구조

```text
src/tool_proxy_agent/integrations/
  llm/                  # LLM 클라이언트 래퍼
...
| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/tool_proxy_agent/integrations/llm` | LLM 연동 | LangChain 모델 래핑, 호출 로깅, 예외 표준화 |
| `src/tool_proxy_agent/integrations/db/base` | DB 공통 타입 | 엔진/세션/모델/쿼리 계약 |
| `src/tool_proxy_agent/integrations/db/query_builder` | 쿼리 빌더 | 읽기/쓰기/삭제 쿼리 조립 유틸 |
| `src/tool_proxy_agent/integrations/db/engines/sqlite` | SQLite 엔진 | 기본 Chat 저장소/로컬 DB 엔진 |
| `src/tool_proxy_agent/integrations/db/engines/postgres` | PostgreSQL 엔진 | 관계형 조회 및 벡터 저장소 지원 |
| `src/tool_proxy_agent/integrations/db/engines/mongodb` | MongoDB 엔진 | 문서 저장/조회 어댑터 |
| `src/tool_proxy_agent/integrations/db/engines/redis` | Redis 엔진 | keyspace 기반 저장/벡터 유틸 |
| `src/tool_proxy_agent/integrations/db/engines/elasticsearch` | Elasticsearch 엔진 | 인덱스 기반 검색 어댑터 |
| `src/tool_proxy_agent/integrations/db/engines/lancedb` | LanceDB 엔진 | 로컬 벡터 저장소 지원 |
| `src/tool_proxy_agent/integrations/fs` | 파일 시스템 연동 | 파일 저장소 계약과 로컬 파일 엔진 구현 |
| `src/tool_proxy_agent/integrations/embedding` | 임베딩 연동 | 임베딩 모델 호출 래퍼 |

### 6-7. `src/tool_proxy_agent/resources`, `static`

| 경로 | 역할 | 구현 책임 범위 |
| --- | --- | --- |
| `src/tool_proxy_agent/resources/dev` | 개발 환경 리소스 | `ENV=dev`일 때 사용할 `.env.sample` 및 환경 파일 보관 |
| `src/tool_proxy_agent/resources/stg` | 스테이징 환경 리소스 | `ENV=stg`용 샘플/설정 보관 |
| `src/tool_proxy_agent/resources/prod` | 운영 환경 리소스 | `ENV=prod`용 샘플/설정 보관 |
| `src/tool_proxy_agent/static/index.html` | UI 엔트리 | 채팅 화면 골격 |
| `src/tool_proxy_agent/static/css` | 스타일 | 레이아웃/테마 정의 |
| `src/tool_proxy_agent/static/js/core` | 프런트 초기화 | 앱 부트스트랩과 전역 흐름 제어 |
| `src/tool_proxy_agent/static/js/chat` | 채팅 UI | 전송, 셀 렌더링, 스트림 반영 |
| `src/tool_proxy_agent/static/js/ui` | UI 제어 | 패널 토글, 그리드, 테마 관리 |
| `src/tool_proxy_agent/static/js/utils` | 프런트 유틸 | DOM, Markdown, 문법 하이라이팅 |
| `src/tool_proxy_agent/static/asset` | 정적 자산 | 아이콘, 로고 등 시각 리소스 |

## 7. 계층 경계

1. `api`는 HTTP 입출력 경계만 다루고, Agent 규칙은 직접 구현하지 않는다.
2. `core`는 tool selector + retry 정책과 그래프 분기를 정의하지만, 큐/버퍼/저장소 세부 구현은 모른다.
...
1. 상단 우측 헤더에는 `기록 초기화`, `새 채팅 생성`, `테마 전환` 버튼이 있다.
2. `기록 초기화` 버튼은 `DELETE /ui-api/chat/sessions`를 호출해 전체 세션을 삭제한 뒤 새 세션 1개를 다시 만든다.
3. 히스토리 패널의 휴지통 버튼은 `DELETE /ui-api/chat/sessions/{session_id}`를 호출해 단건 세션만 삭제한다.
