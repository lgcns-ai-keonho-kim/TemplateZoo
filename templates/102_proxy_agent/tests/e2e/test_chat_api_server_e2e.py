...
디자인 패턴: 블랙박스 E2E 테스트
참조: tests/conftest.py, src/tool_proxy_agent/api/chat/routers/router.py
"""
...
from tool_proxy_agent.core.chat.const.messages import SafeguardRejectionMessage
        ...
        collected.append(
            {
                ...
                "tool_call_id": str(metadata_dict.get("tool_call_id") or ""),
                "attempt": str(metadata_dict.get("attempt") or ""),
...
def test_clear_all_sessions_via_ui_api(chat_api_client: httpx.Client) -> None:
    """UI API로 전체 세션을 삭제하면 목록이 비워지는지 확인한다."""

    first_session_id = _create_session(chat_api_client, "전체 삭제 테스트 1")
    second_session_id = _create_session(chat_api_client, "전체 삭제 테스트 2")

    clear_response = chat_api_client.delete("/ui-api/chat/sessions")
    assert clear_response.status_code == 200, clear_response.text

    cleared = _require_json_payload(clear_response)
    assert cleared["deleted"] is True
    assert int(cleared["deleted_count"]) >= 2
    ...
    first_messages_response = chat_api_client.get(
        f"/ui-api/chat/sessions/{first_session_id}/messages",
        params={"limit": 20, "offset": 0},
    )
    assert first_messages_response.status_code == 404, first_messages_response.text

    second_messages_response = chat_api_client.get(
        f"/ui-api/chat/sessions/{second_session_id}/messages",
        params={"limit": 20, "offset": 0},
    )
    assert second_messages_response.status_code == 404, second_messages_response.text
