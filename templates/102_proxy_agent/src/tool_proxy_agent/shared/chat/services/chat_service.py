...
디자인 패턴: 서비스 레이어
참조: src/tool_proxy_agent/shared/chat/graph/base_chat_graph.py, src/tool_proxy_agent/core/chat/graphs/chat_graph.py
"""
...
from tool_proxy_agent.core.chat.const import DEFAULT_CONTEXT_WINDOW
from tool_proxy_agent.core.chat.models import (
    ChatMessage,
    ChatRole,
    ChatSession,
)
from tool_proxy_agent.shared.chat.interface import ChatServicePort, GraphPort
from tool_proxy_agent.shared.chat.memory import ChatSessionMemoryStore
from tool_proxy_agent.shared.chat.repositories import ChatHistoryRepository
        ...
        for event in self._graph.stream_events(
            ...
            if event_name == "assistant_message":
                ...
                if candidate.strip():
                    ...
                    done_node = node or done_node
            if event_name == "rag_references":
                if isinstance(data, list):
                    rag_references = [item for item in data if isinstance(item, dict)]
                continue
