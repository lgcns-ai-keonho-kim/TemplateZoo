...
디자인 패턴: 서비스 레이어
참조: src/tool_proxy_agent/shared/chat/services/chat_service.py
"""

from __future__ import annotations

from tool_proxy_agent.api.ui.models import (
...
from tool_proxy_agent.api.ui.utils.mappers import (
    to_ui_message,
    to_ui_session,
)
from tool_proxy_agent.shared.chat import ChatService
...
class ChatUIService:
    ...
    def clear_all_sessions(self) -> UIClearSessionsResponse:
        """UI 전체 세션 삭제 요청을 처리한다."""

        deleted_count = 0
        while True:
            sessions = self._chat_service.list_sessions(limit=200, offset=0)
            if not sessions:
                break
            for session in sessions:
                self._chat_service.delete_session(session_id=session.session_id)
                deleted_count += 1
        return UIClearSessionsResponse(deleted=True, deleted_count=deleted_count)
