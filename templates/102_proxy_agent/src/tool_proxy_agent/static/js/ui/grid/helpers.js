...
(function () {
  ...
  function normalizeMessagesResponse(response) {
    var source = response && Array.isArray(response.messages) ? response.messages : [];
    var normalized = [];
    source.forEach(function (message) {
      ...
      var roleRaw = String(message.role || '').toLowerCase();
      var role = roleRaw === 'assistant' ? 'assistant' : 'user';
      normalized.push({
        role: role,
        content: content
      });
  ...
  function setHeaderActionButtonState(buttonId, disabled) {
    var button = window.App.utils.qs(buttonId);
    if (!button) {
      return;
    }
    if (disabled) {
  ...
  function runLocked(task) {
    ...
    setHeaderActionButtonState('#addCell', true);
    setHeaderActionButtonState('#clearHistory', true);
    return Promise.resolve()
      .then(task)
      .finally(function () {
        window.App.grid.isLoading = false;
        setHeaderActionButtonState('#addCell', false);
        setHeaderActionButtonState('#clearHistory', false);
      });
  }
