...
(function () {
  ...
  window.App.grid.init = function () {
    ...
    var clearBtn = window.App.utils.qs('#clearHistory');
    if (!grid) {
      return;
    }
    ...
    if (clearBtn) {
      clearBtn.addEventListener('click', function () {
        window.App.grid.clearAllSessions();
      });
    }
  ...
  window.App.grid.clearAllSessions = function () {
    return helpers.runLocked(function () {
      return window.App.grid._clearAllSessionsInternal();
    }).catch(function (error) {
      console.error('[ui] 전체 세션 삭제 실패', error);
      showUiError(error, '대화 기록 전체 삭제에 실패했습니다.');
      return null;
    });
  };
  ...
  window.App.grid._clearAllSessionsInternal = function () {
    if (!window.confirm('대화 기록을 모두 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
    ...
    return window.App.apiTransport
      ...
      .then(function () {
        ...
        helpers.destroyActiveCell();
        window.App.grid.activeSessionId = null;
        if (window.App.app && typeof window.App.app.clearAllHistory === 'function') {
          window.App.app.clearAllHistory();
        }
        return window.App.grid._createNewSessionInternal();
      });
  };
