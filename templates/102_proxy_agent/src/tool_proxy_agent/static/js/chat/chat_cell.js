/*
  ...
  참조: js/chat/cell/config.js, js/chat/cell/scroll.js, js/chat/cell/references.js, js/chat/cell/tool_trace.js, js/chat/cell/stream.js
*/
(function () {
  'use strict';
  ...
  var toolTraceUtil = window.App.chatCellToolTrace;
  var streamUtil = window.App.chatCellStream;
  if (!scrollUtil || !referencesUtil || !toolTraceUtil || !streamUtil) {
  ...
  window.App.chatCell.create = function (cellId, title, options) {
    ...
    function createToolTraceController(messageBubble) {
      if (!messageBubble || typeof toolTraceUtil.create !== 'function') {
        return null;
      }
      try {
        return toolTraceUtil.create(messageBubble);
      } catch (error) {
        console.warn('[ui] tool trace 컨트롤러 생성 실패', error);
        return null;
      }
    }
    function applyToolEvent(eventType, payload) {
      if (!state.toolTraceController || typeof state.toolTraceController.apply !== 'function') {
        return;
      }
      state.toolTraceController.apply(eventType, payload);
      scrollMessagesToBottom(isAutoScrollFollowing());
    }
    ...
    function finishSending() {
      ...
      state.toolTraceController = null;
      state.receivedText = '';
      state.references = [];
    ...
    function finalizeError(message) {
      ...
      state.finalized = true;
      var safeMessage = presenter && typeof presenter.normalizeErrorMessage === 'function'
        ? presenter.normalizeErrorMessage(message, '응답 생성에 실패했습니다.')
        : (message && String(message).trim() ? String(message).trim() : '응답 생성에 실패했습니다.');
    ...
    function handleSend() {
      ...
      status.classList.add('is-visible');
      presenter.setStatus(status, '작업 요청 등록중...', true, 'QUEUE', true);
      ...
      state.toolTraceController = createToolTraceController(assistantMessage.bubble);
      state.tokenBuffer = '';
      state.receivedText = '';
      state.references = [];
    ...
    function destroy() {
      ...
      detachStream();
      presenter.setSendingState(state, false, sendBtn, stopBtn, input);
      if (state.toolTraceController && typeof state.toolTraceController.destroy === 'function') {
        state.toolTraceController.destroy();
      }
      state.activeBubble = null;
      state.activeMessageNode = null;
      state.toolTraceController = null;
