...
| 경로 | 역할 |
| --- | --- |
| `src/tool_proxy_agent/integrations/db/engines/mongodb/engine.py` | MongoDB 엔진 본체 |
| `src/tool_proxy_agent/integrations/db/engines/mongodb/connection.py` | 연결/DB 객체 초기화 |
| `src/tool_proxy_agent/integrations/db/engines/mongodb/schema_manager.py` | 컬렉션 생성/필드 변경 |
| `src/tool_proxy_agent/integrations/db/engines/mongodb/filter_builder.py` | Query -> Mongo 필터 변환 |
| `src/tool_proxy_agent/api/chat/services/runtime.py` | 저장소 조립 지점 |
| `src/tool_proxy_agent/shared/chat/repositories/history_repository.py` | `db_client` 주입형 저장소 |
...
from tool_proxy_agent.shared.chat import ChatHistoryRepository
