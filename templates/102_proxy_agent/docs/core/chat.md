...
`src/tool_proxy_agent/core/chat`의 현재 실행 구조(Plan-and-then-Execute)를 코드 기준으로 정리한다.
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 엔티티 | `src/tool_proxy_agent/core/chat/models/entities.py` | `ChatSession`, `ChatMessage`, `ChatRole` 정의 |
| 상수 | `src/tool_proxy_agent/core/chat/const/settings.py` | DB 경로, 페이지, 문맥 길이 기본값 |
| 상태 | `src/tool_proxy_agent/core/chat/state/graph_state.py` | 그래프 상태 키 정의 |
| 그래프 | `src/tool_proxy_agent/core/chat/graphs/chat_graph.py` | 노드 등록, 엣지, stream 정책 |
| 노드 | `src/tool_proxy_agent/core/chat/nodes/*.py` | safeguard/planner/execute/replan/response 조립 |
| Plan 유틸 공개 API | `src/tool_proxy_agent/core/chat/nodes/_plan_utils.py` | Plan 유틸 단일 진입점(파사드) |
| Plan 유틸 구현 | `src/tool_proxy_agent/core/chat/nodes/_plan_*.py` | 파싱/정규화/의존성 검증/요약 세부 구현 |
| 프롬프트 | `src/tool_proxy_agent/core/chat/prompts/*.py` | 시스템 프롬프트 |
| Tool 구현 | `src/tool_proxy_agent/core/chat/tools/*.py` | 기본 Tool 함수 구현 |
| Tool Registry | `src/tool_proxy_agent/core/chat/tools/registry.py` | ToolRegistry singleton 생성/기본 Tool 등록 |
| 매퍼 | `src/tool_proxy_agent/core/chat/utils/mapper.py` | 도메인 모델과 DB 문서 변환 |

## 3. ChatGraphState 핵심 키

`src/tool_proxy_agent/core/chat/state/graph_state.py` 기준 주요 키:
...
`src/tool_proxy_agent/core/chat/graphs/chat_graph.py` 기준:
...
1. Tool 함수 구현: `src/tool_proxy_agent/core/chat/tools/*.py`
2. Tool 등록/단일 singleton 관리: `src/tool_proxy_agent/core/chat/tools/registry.py`
3. Tool 계약/레지스트리 타입: `src/tool_proxy_agent/shared/chat/tools/*.py`
4. Tool 실행 노드: `src/tool_proxy_agent/shared/chat/nodes/tool_exec_node.py`
5. Tool 실행 보조 로직: `src/tool_proxy_agent/shared/chat/nodes/_tool_exec_support.py`

## 5-1. Plan 유틸 분리 구조

`src/tool_proxy_agent/core/chat/nodes/_plan_utils.py` 기준 공개 함수는 그대로 유지되고, 구현만 분리됐다.
...
`src/tool_proxy_agent/shared/chat/tools/registry.py` 기준 공개 메서드:

1. 등록/조회: `add_tool`, `register_spec`, `resolve`, `has`
2. 목록 조회: `get_tools`(읽기 전용 tuple), `list_specs`(하위 호환 list), `list_for_planner`

`src/tool_proxy_agent/core/chat/nodes/planner_tools_payload_node.py` 기준:
