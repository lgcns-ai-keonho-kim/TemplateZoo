...
`src/tool_proxy_agent/shared/const` 전역 상수의 의미와 사용 원칙을 정리한다.

## 1. 용어 정리

| 용어 | 의미 | 관련 스크립트 |
| --- | --- | --- |
| SharedConst | 전역 공통 상수 클래스 | `src/tool_proxy_agent/shared/const/__init__.py` |
...
| 파일 | 역할 |
| --- | --- |
| `src/tool_proxy_agent/shared/const/__init__.py` | 공통 상수 정의 |
| `src/tool_proxy_agent/shared/config/loader.py` | 중첩 구분자와 기본 인코딩 사용 |
