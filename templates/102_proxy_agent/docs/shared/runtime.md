...
`src/tool_proxy_agent/shared/runtime`의 Queue, EventBuffer, Worker, ThreadPool 구성요소를 코드 기준으로 정리한다.
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 런타임 공개 API | `src/tool_proxy_agent/shared/runtime/__init__.py` | Queue/Buffer/Worker/ThreadPool 공개 |
| Queue 모델 | `src/tool_proxy_agent/shared/runtime/queue/model.py` | `QueueConfig`, `QueueItem` |
| InMemory Queue | `src/tool_proxy_agent/shared/runtime/queue/in_memory_queue.py` | 프로세스 내부 큐 |
| Redis Queue | `src/tool_proxy_agent/shared/runtime/queue/redis_queue.py` | Redis 리스트 기반 큐 |
| Buffer 모델 | `src/tool_proxy_agent/shared/runtime/buffer/model.py` | `EventBufferConfig`, `StreamEventItem` |
| InMemory Buffer | `src/tool_proxy_agent/shared/runtime/buffer/in_memory_buffer.py` | 요청 버킷 기반 이벤트 버퍼 |
| Redis Buffer | `src/tool_proxy_agent/shared/runtime/buffer/redis_buffer.py` | Redis 리스트 기반 이벤트 버퍼 |
| Worker 모델 | `src/tool_proxy_agent/shared/runtime/worker/model.py` | `WorkerConfig`, `WorkerState` |
| Worker 구현 | `src/tool_proxy_agent/shared/runtime/worker/worker.py` | 큐 소비 워커 |
| ThreadPool 모델 | `src/tool_proxy_agent/shared/runtime/thread_pool/model.py` | `ThreadPoolConfig`, `TaskRecord` |
| ThreadPool 구현 | `src/tool_proxy_agent/shared/runtime/thread_pool/thread_pool.py` | 태스크 제출/종료 |

연동 스크립트:

1. `src/tool_proxy_agent/api/chat/services/runtime.py`
2. `src/tool_proxy_agent/shared/chat/services/service_executor.py`
