...
`src/tool_proxy_agent/static`의 프런트엔드 동작, API 연동, 상태 전이 규칙을 정리한다.
...
1. `src/tool_proxy_agent/static/index.html`
2. `src/tool_proxy_agent/static/js/core/app.js`
3. `src/tool_proxy_agent/static/js/ui/grid_manager.js`
4. `src/tool_proxy_agent/static/js/chat/api_transport.js`
5. `src/tool_proxy_agent/static/js/chat/chat_cell.js`
6. `src/tool_proxy_agent/static/js/chat/chat_presenter.js`
...
```json
{
  "session_id": "...",
  "request_id": "...",
  "type": "start|token|references|tool_start|tool_result|tool_error|done|error",
  "node": "executor|rag|tool_exec|response|blocked",
  "content": "...",
...
BE-->>API: token*
BE-->>API: references
BE-->>API: tool_start/tool_result/tool_error*
...
## 6-4. tool trace UI 동작

1. `type=tool_start|tool_result|tool_error` 이벤트를 수신하면 assistant 메시지에서 본문 버블 바깥 상단에 도구 실행 카드가 렌더링된다.
2. `tool_start`는 카드 생성/실행중 상태를 먼저 표시한다.
3. `tool_result`/`tool_error`는 동일 `step_id` 카드를 성공/실패 상태로 갱신한다.
4. 카드에는 `tool_name`, 상태, 결과/오류 요약만 표시한다.
5. `step_id`, `plan_id`, `attempt`, `duration_ms`, `ok`, `error_code`는 내부 추적 용도로만 사용하며 UI에는 노출하지 않는다.
6. 도구 카드는 스트림 실시간 표시 전용이며, 세션 재진입 시 메시지 이력에서 복원하지 않는다.
...
`chat_cell.js` 내부 `connectStream(...)` + `js/chat/cell/stream.js`의 `connect(...)` 기준.

1. `POST /chat` 응답은 `session_id`, `request_id`, `status=QUEUED`를 모두 만족해야만 스트림 연결을 진행한다.
2. `onQueued`에서 `activeRequestId`를 갱신한다.
3. `onEvent`에서 이벤트 타입 화이트리스트(`start|token|references|tool_start|tool_result|tool_error|done|error`)를 통과하지 못하면 즉시 오류 종료한다.
4. `request_id` 검증은 2단계다: transport(`js/chat/transport/stream.js`)에서 제출 응답의 `request_id`와 불일치하면 프로토콜 오류로 종료하고, cell(`js/chat/cell/stream.js`)에서는 stale 이벤트를 한 번 더 무시한다.
5. `token`은 `node === "response"`일 때만 본문 버퍼에 누적한다.
6. `tool_*` 이벤트는 assistant 본문 버블보다 먼저(바깥 상단 카드) 반영하고, 상태 배지(`status`)도 갱신한다.
7. `done`은 `payload.content` 우선, 비어 있으면 `tokenBuffer`를 사용한다.
8. `done.status`가 존재하면 `COMPLETED|FAILED`만 허용한다.
9. `error`는 `error_message`를 우선 표시한다.
10. 네트워크/연결 오류는 `tokenBuffer`가 있으면 성공 완료로 마감할 수 있다.
11. 계약 위반(예: SSE payload 형식 오류, 지원하지 않는 이벤트 타입, 제출 응답 형식 불일치)은 `tokenBuffer`가 있어도 실패로 마감한다.
12. 오류 메시지는 사용자 친화 문구로 정규화해 표시한다(네트워크/404/503/504/500 등).

## 8. 실패 처리 매트릭스

| 구간 | 실패 조건 | 처리 |
| --- | --- | --- |
| 세션 목록 로딩 | `listSessions` 실패 | 새 세션 생성 경로로 fallback, fallback도 실패하면 alert 표시 |
| 메시지 목록 로딩 | `listMessages` 실패 또는 형식 불일치 | 빈 배열 렌더 |
| 요청 제출 | `POST /chat` 실패 또는 제출 응답 형식 불일치 | assistant placeholder를 실패 상태로 표시 |
| SSE 파싱 | payload 형식 오류 또는 이벤트 타입 불일치 | `onError` 경로로 종료 |
| SSE 연결 | 네트워크 오류 | tokenBuffer 존재 시 성공 마감, 없으면 실패 |
| SSE 프로토콜 | payload/이벤트/제출 응답 계약 위반 | tokenBuffer 존재 여부와 무관하게 실패 처리 |
...
| `api_transport.js` | 프레임워크용 전송 어댑터 모듈(신규 작성) | 프레임워크용 전송 어댑터 모듈(신규 작성) |
...
1. 현재 정적 UI는 `js/chat/api_transport.js` 단일 퍼사드를 사용한다.
2. 프레임워크 전환 시 필요하면 UI API와 Chat API 어댑터 모듈을 분리해 신규 작성한다.
...
| 키 | 의미 | 값 |
| --- | --- | --- |
| `tool_proxy_agent-theme` | 테마 유지 | `light` / `dark` |
| `tool_proxy_agent-panel-collapsed` | 패널 접힘 여부 | `true` / `false` |
