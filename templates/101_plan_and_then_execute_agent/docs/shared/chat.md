...
이 문서는 `src/plan_and_then_execute_agent/shared/chat` 모듈의 실행 흐름, 저장 규칙, 변경 지점을 코드 기준으로 정리한다.
...
```text
src/plan_and_then_execute_agent/shared/chat/
  interface/
  graph/
  memory/
  nodes/
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 포트 | `src/plan_and_then_execute_agent/shared/chat/interface/ports.py` | Graph/Service/Executor 인터페이스 정의 |
| 그래프 실행 | `src/plan_and_then_execute_agent/shared/chat/graph/base_chat_graph.py` | 그래프 컴파일/실행/이벤트 필터링 |
| 메모리 캐시 | `src/plan_and_then_execute_agent/shared/chat/memory/session_store.py` | 세션 메시지 캐시 rpush/lrange |
| 공통 노드 | `src/plan_and_then_execute_agent/shared/chat/nodes/llm_node.py` | LLM 호출 노드 구현 |
| 공통 노드 | `src/plan_and_then_execute_agent/shared/chat/nodes/branch_node.py` | 분기 노드 구현 |
| 공통 노드 | `src/plan_and_then_execute_agent/shared/chat/nodes/message_node.py` | 메시지 선택 노드 구현 |
| 저장소 | `src/plan_and_then_execute_agent/shared/chat/repositories/history_repository.py` | 세션/메시지/커밋 저장 |
| 스키마 | `src/plan_and_then_execute_agent/shared/chat/repositories/schemas/*.py` | DB 컬렉션 스키마 생성 |
| 서비스 | `src/plan_and_then_execute_agent/shared/chat/services/chat_service.py` | 도메인 유스케이스 실행 |
| 실행기 | `src/plan_and_then_execute_agent/shared/chat/services/service_executor.py` | 실행 오케스트레이션과 공개 동작 |
| 실행기 보조 | `src/plan_and_then_execute_agent/shared/chat/services/_service_executor_support.py` | 이벤트 정규화/영속 후처리/상태 보조 로직 |

연결 스크립트:

1. `src/plan_and_then_execute_agent/core/chat/graphs/chat_graph.py`
2. `src/plan_and_then_execute_agent/api/chat/services/runtime.py`
3. `src/plan_and_then_execute_agent/api/chat/routers/*.py`
4. `src/plan_and_then_execute_agent/api/ui/services/chat_service.py`
...
sequenceDiagram
    ...
    EX->>Q: get(job)
    EX->>SVC: astream(session_id, user_query, context_window)
    SVC->>G: astream_events(...)
    G-->>SVC: node/event/data
    SVC-->>EX: normalized events
    EX->>BUF: push(start/token/references/tool_*/done/error)
...
### 3-4. tool_* payload와 표시 경계

`service_executor.py` + `_service_executor_support.py` 기준으로 `tool_start/tool_result/tool_error`는 아래 정보를 포함해 정규화된다.

1. `data`: Tool 실행 envelope(`tool_name`, `step_id`, `plan_id`, `attempt`, `duration_ms`, `output`, `error`, `error_code`)
2. `metadata`: 최소 `tool_name`, `step_id`, `plan_id` 병합

운영 규칙:

1. SSE 계약에는 추적 필드가 포함될 수 있으나, UI는 사용자 정보 중심으로 표시해야 한다.
2. 내부 추적 필드(`step_id`, `plan_id`, `attempt`, `duration_ms`, `ok`, `error_code`)는 화면 비노출 원칙을 유지한다.
...
조립 파일: `src/plan_and_then_execute_agent/api/chat/services/runtime.py`
...
2. 이벤트 타입 집합이 `start/token/references/tool_start/tool_result/tool_error/done/error`와 일치하는가
3. 환경 변수 기본값이 `runtime.py` 코드와 일치하는가
4. request_id 멱등 저장 흐름이 `chat_service.py`/`history_repository.py`와 일치하는가
5. tool_* payload의 추적 필드와 UI 비노출 원칙이 일치하는가
6. 문서의 경로가 실제 `src/plan_and_then_execute_agent/shared/chat` 파일과 일치하는가
