...
`src/plan_and_then_execute_agent/shared/exceptions`의 공통 예외 모델과 사용 규칙을 코드 기준으로 정리한다.
...
| 파일 | 역할 |
| --- | --- |
| `src/plan_and_then_execute_agent/shared/exceptions/base.py` | 공통 예외 클래스 정의 |
| `src/plan_and_then_execute_agent/shared/exceptions/models.py` | 상세 모델 정의 |
| `src/plan_and_then_execute_agent/shared/exceptions/__init__.py` | 공개 API 제공 |
| `src/plan_and_then_execute_agent/api/chat/routers/common.py` | Chat API 예외를 HTTP 상태로 매핑 |
| `src/plan_and_then_execute_agent/api/ui/routers/common.py` | UI API 예외를 HTTP 상태로 매핑 |
