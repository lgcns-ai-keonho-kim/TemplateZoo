...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 라우터 집계 | `src/plan_and_then_execute_agent/api/chat/routers/router.py` | Chat 하위 라우터 등록 |
| 작업 제출 | `src/plan_and_then_execute_agent/api/chat/routers/create_chat.py` | `POST /chat` 처리 |
| 이벤트 구독 | `src/plan_and_then_execute_agent/api/chat/routers/stream_chat_events.py` | `GET /chat/{session_id}/events` SSE 중계 |
| 세션 스냅샷 | `src/plan_and_then_execute_agent/api/chat/routers/get_chat_session.py` | `GET /chat/{session_id}` 처리 |
| 요청/응답 DTO | `src/plan_and_then_execute_agent/api/chat/models/stream.py` | Submit/Stream 모델 |
| 실행 런타임 | `src/plan_and_then_execute_agent/api/chat/services/runtime.py` | ChatService, ServiceExecutor 조립 |
| 실행 오케스트레이터 | `src/plan_and_then_execute_agent/shared/chat/services/service_executor.py` | 큐 소비, 이벤트 변환, SSE payload 생성 |
