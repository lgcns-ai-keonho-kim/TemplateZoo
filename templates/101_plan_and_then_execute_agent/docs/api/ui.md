...
| UI 서비스 | Core ChatService 호출 결과를 UI DTO로 변환하는 계층 | `src/plan_and_then_execute_agent/api/ui/services/chat_service.py` |
| UI DTO | UI에서 직접 소비하는 요청/응답 모델 | `src/plan_and_then_execute_agent/api/ui/models/*.py` |
| 매퍼 | Core 엔티티를 UI DTO로 바꾸는 순수 함수 | `src/plan_and_then_execute_agent/api/ui/utils/mappers.py` |
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 라우터 집계 | `src/plan_and_then_execute_agent/api/ui/routers/router.py` | `/ui-api/chat` prefix 라우터 조립 |
| 세션 생성 | `src/plan_and_then_execute_agent/api/ui/routers/create_session.py` | `POST /sessions` |
| 세션 목록 | `src/plan_and_then_execute_agent/api/ui/routers/list_sessions.py` | `GET /sessions` |
| 메시지 목록 | `src/plan_and_then_execute_agent/api/ui/routers/list_messages.py` | `GET /sessions/{session_id}/messages` |
| 세션 삭제 | `src/plan_and_then_execute_agent/api/ui/routers/delete_session.py` | `DELETE /sessions/{session_id}` |
| 예외 변환 | `src/plan_and_then_execute_agent/api/ui/routers/common.py` | `BaseAppException` HTTP 변환 |
| 서비스 | `src/plan_and_then_execute_agent/api/ui/services/chat_service.py` | 조회/생성/삭제 유스케이스 조합 |
| 서비스 DI | `src/plan_and_then_execute_agent/api/ui/services/__init__.py` | `get_chat_ui_service()` 제공 |
| 모델 | `src/plan_and_then_execute_agent/api/ui/models/session.py`, `src/plan_and_then_execute_agent/api/ui/models/message.py` | 요청/응답 DTO |
| 매퍼 | `src/plan_and_then_execute_agent/api/ui/utils/mappers.py` | `to_ui_session`, `to_ui_message` |
| 경로 상수 | `src/plan_and_then_execute_agent/api/const/chat.py` | UI API prefix/path |
| 페이지 상수 | `src/plan_and_then_execute_agent/core/chat/const/settings.py` | `DEFAULT_PAGE_SIZE=50`, `MAX_PAGE_SIZE=200` |
...
`src/plan_and_then_execute_agent/api/ui/routers/common.py` 기준:
...
`src/plan_and_then_execute_agent/static/js/ui/grid_manager.js` 기준:
