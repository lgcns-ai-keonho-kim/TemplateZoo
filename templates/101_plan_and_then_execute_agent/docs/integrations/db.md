...
이 문서는 `src/plan_and_then_execute_agent/integrations/db`의 공통 인터페이스, Query DSL, 엔진별 차이, 교체 절차를 코드 기준으로 정리한다.
...
| 분류 | 파일 | 역할 |
| --- | --- | --- |
| 공개 API | `src/plan_and_then_execute_agent/integrations/db/__init__.py` | DBClient/Builder/Engine 노출 |
| 클라이언트 | `src/plan_and_then_execute_agent/integrations/db/client.py` | 엔진 연결, 스키마 등록, CRUD, 벡터 검색 호출 |
| 베이스 인터페이스 | `src/plan_and_then_execute_agent/integrations/db/base/engine.py` | 엔진 공통 메서드 정의 |
| 베이스 모델 공개 API | `src/plan_and_then_execute_agent/integrations/db/base/models.py` | DB 모델 단일 진입점(파사드) |
| 베이스 모델 구현 | `src/plan_and_then_execute_agent/integrations/db/base/_*.py` | 엔티티 단위 모델 구현 파일 |
| 공통 DSL | `src/plan_and_then_execute_agent/integrations/db/base/query_builder.py` | where/order/limit/vector DSL |
| 읽기 DSL | `src/plan_and_then_execute_agent/integrations/db/query_builder/read_builder.py` | `fetch`, `fetch_vector` |
| 쓰기 DSL | `src/plan_and_then_execute_agent/integrations/db/query_builder/write_builder.py` | `upsert`, `upsert_one` |
| 삭제 DSL | `src/plan_and_then_execute_agent/integrations/db/query_builder/delete_builder.py` | `by_id`, `execute` |
| LanceDB 엔진 | `src/plan_and_then_execute_agent/integrations/db/engines/lancedb/engine.py` | 파일 기반 벡터 저장/검색 |
| SQLite 엔진 | `src/plan_and_then_execute_agent/integrations/db/engines/sqlite/engine.py` | 로컬 기본 RDB 엔진 |
| Postgres 엔진 | `src/plan_and_then_execute_agent/integrations/db/engines/postgres/engine.py` | PostgreSQL + pgvector |
| Redis 엔진 | `src/plan_and_then_execute_agent/integrations/db/engines/redis/engine.py` | Redis 기반 저장/검색 |
| MongoDB 엔진 | `src/plan_and_then_execute_agent/integrations/db/engines/mongodb/engine.py` | MongoDB CRUD |
| Elasticsearch 엔진 | `src/plan_and_then_execute_agent/integrations/db/engines/elasticsearch/engine.py` | ES 검색/벡터 검색 |

### 2-1. 베이스 모델 분리 구조

`base/models.py` 공개 함수/클래스 이름은 유지되며, 구현은 아래 파일로 분리되었다.

1. 스키마: `_column_spec.py`, `_collection_schema.py`
2. 문서/벡터: `_document.py`, `_vector.py`
3. 필터: `_field_source.py`, `_filter_operator.py`, `_filter_condition.py`, `_filter_expression.py`
4. 정렬/조회: `_sort_order.py`, `_sort_field.py`, `_pagination.py`, `_query.py`
5. 벡터 검색: `_vector_search_request.py`, `_vector_search_result.py`, `_vector_search_response.py`, `_collection_info.py`
...
| payload 필터 조회 실패 | payload_field 미정의 | `base/models.py`, `_collection_schema.py` | 스키마 payload_field 설정 확인 |
...
3. pagination 기본값이 `base/models.py`와 `_pagination.py`와 일치하는가
4. 엔진 제약 설명이 각 `engine.py` 구현과 일치하는가
5. 문서 경로가 실제 `src/plan_and_then_execute_agent/integrations/db` 구조와 일치하는가
