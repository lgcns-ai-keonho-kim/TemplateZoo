...
(function () {
  ...
  function normalizeToolEvent(eventType, payload, fallbackStepId) {
    ...
    var stepId = toText(contentObj.step_id || metadata.step_id || fallbackStepId);
  ...
  function createCard(stepId, toolName) {
    var root = createEl('article', 'tool-trace-card');
    root.setAttribute('data-step-id', stepId);
  ...
  function createController(bubbleEl) {
    ...
    function ensureCard(model) {
      var key = model.stepId || ('step-unknown-' + String(fallbackIndex));
      var existing = cardsByStepId[key];
      if (existing) {
        return existing;
      }
      var created = createCard(key, model.toolName);
      cardsByStepId[key] = created;
    ...
    function apply(eventType, payload) {
      ...
      fallbackIndex += 1;
      var model = normalizeToolEvent(eventType, payload, 'step-unknown-' + String(fallbackIndex));
