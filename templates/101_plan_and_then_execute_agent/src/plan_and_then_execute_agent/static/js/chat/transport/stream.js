...
(function () {
  ...
  function normalizeQueuedResponse(queued) {
    if (!queued || typeof queued !== 'object') {
      throw new Error('작업 제출 응답 형식이 올바르지 않습니다.');
    }
    var sessionId = String(queued.session_id || '').trim();
    var requestId = String(queued.request_id || '').trim();
    var status = String(queued.status || '').trim().toUpperCase();
    if (!sessionId || !requestId) {
      throw new Error('작업 제출 응답에 session_id 또는 request_id가 없습니다.');
    }
    if (status !== 'QUEUED') {
      throw new Error('작업 제출 상태가 QUEUED가 아닙니다.');
    }
    return {
  ...
  function createStreamHandle(sessionId, message, contextWindow, handlers) {
    ...
    function validatePayload(payload) {
      ...
      if (typeof payload.session_id !== 'string' || !payload.session_id.trim()) {
        throw new Error('SSE payload의 session_id 필드가 누락되었습니다.');
      }
      if (typeof payload.type !== 'string' || !payload.type.trim()) {
        throw new Error('SSE payload의 type 필드가 누락되었습니다.');
      }
      if (!ALLOWED_EVENT_TYPES[payload.type]) {
        throw new Error('지원하지 않는 SSE 이벤트 타입입니다: ' + payload.type);
      }
    ...
    http.requestJson('POST', '/chat', submitPayload)
      .then(function (queued) {
        ...
        var normalized = normalizeQueuedResponse(queued);
        activeSessionId = normalized.session_id;
        activeRequestId = normalized.request_id;
        if (handlers && typeof handlers.onQueued === 'function') {
          handlers.onQueued(normalized);
        }
  ...
  window.App.apiTransportStream = {
    ...
    allowedEventTypes: Object.keys(ALLOWED_EVENT_TYPES)
  };
})();
