...
디자인 패턴: 어댑터
참조: src/plan_and_then_execute_agent/shared/chat/tools/types.py
"""
...
def api_agent_response(tool_call: ToolCall) -> ToolResult:
    ...
    base_url = str(args.get("base_url") or AGENT_API_BASE_URL).strip() or AGENT_API_BASE_URL
    session_id = str(args.get("session_id") or "").strip()

    context_window_raw = args.get("context_window")
    context_window = int(context_window_raw) if context_window_raw is not None else DEFAULT_CONTEXT_WINDOW
    if context_window <= 0:
        return {
            "ok": False,
            "output": {},
    ...
    connect_timeout = float(connect_timeout_raw) if connect_timeout_raw is not None else DEFAULT_CONNECT_TIMEOUT_SECONDS
    read_timeout = float(read_timeout_raw) if read_timeout_raw is not None else DEFAULT_READ_TIMEOUT_SECONDS
