...
디자인 패턴: 싱글턴 + 모듈 초기화
참조: 
- src/plan_and_then_execute_agent/core/chat/tools/math_tools.py
- src/plan_and_then_execute_agent/core/chat/tools/api_get_weather.py
- src/plan_and_then_execute_agent/core/chat/tools/api_agent_response.py
"""
...
# `name`: planner가 선택하는 고유 tool 식별자
# `description`: planner 프롬프트에 노출되는 기능 설명
# `args_schema`: planner가 생성해야 할 인자 JSON 스키마
