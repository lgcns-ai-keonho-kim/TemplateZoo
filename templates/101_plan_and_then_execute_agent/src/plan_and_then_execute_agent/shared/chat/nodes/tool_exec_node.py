...
디자인 패턴: 실행 오케스트레이터
참조: src/plan_and_then_execute_agent/shared/chat/tools/registry.py, src/plan_and_then_execute_agent/shared/chat/tools/types.py
"""
...
from plan_and_then_execute_agent.shared.chat.nodes._state_adapter import coerce_state_mapping
from plan_and_then_execute_agent.shared.chat.nodes._tool_exec_support import _ToolExecSupportMixin
from plan_and_then_execute_agent.shared.chat.tools import ToolCall, ToolRegistry, ToolResult
...
class ToolExecNode(_ToolExecSupportMixin):
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(registry, ToolRegistry):
            ...
            raise BaseAppException("ToolExecNode registry 설정이 올바르지 않습니다.", detail)
        ...
        self._logger = logger or create_default_logger(f"ToolExecNode:{normalized_node_name}")
    ...
    def _run_sync(self, state: Mapping[str, Any], *, writer: Any | None) -> dict[str, Any]:
        tool_call = self._extract_tool_call(state)
        tool_name = str(tool_call.get("tool_name") or "")
        step_id = str(tool_call.get("step_id") or "")
        plan_id = str(tool_call.get("plan_id") or "")
        ...
        for attempt in range(1, attempt_limit + 1):
            ...
            try:
                ...
                error_message = str(normalized.get("error") or "Tool 실행에 실패했습니다.")
    ...
    async def _run_async(self, state: Mapping[str, Any], *, writer: Any | None) -> dict[str, Any]:
        tool_call = self._extract_tool_call(state)
        tool_name = str(tool_call.get("tool_name") or "")
        step_id = str(tool_call.get("step_id") or "")
        plan_id = str(tool_call.get("plan_id") or "")
    ...
    def _extract_tool_call(self, state: Mapping[str, Any]) -> ToolCall:
        ...
        return {
            ...
            "plan_id": str(raw_tool_call.get("plan_id") or ""),
            "step_id": str(raw_tool_call.get("step_id") or ""),
            "step_index": int(raw_tool_call.get("step_index") or 0),
            "state": state,
        }

    def _invoke_sync_with_timeout(self, *, fn: Any, tool_call: ToolCall, timeout_seconds: float) -> Any:
        ...
        if inspect.isawaitable(result):
            ...
            raise BaseAppException("비동기 Tool 함수는 arun 경로에서 실행해야 합니다.", detail)
        return result

    async def _invoke_async_with_timeout(self, *, fn: Any, tool_call: ToolCall, timeout_seconds: float) -> Any:
        result = fn(tool_call)
