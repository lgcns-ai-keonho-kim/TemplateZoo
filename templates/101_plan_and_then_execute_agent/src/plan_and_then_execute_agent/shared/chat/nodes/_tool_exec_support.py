...
디자인 패턴: 믹스인
참조: src/plan_and_then_execute_agent/shared/chat/nodes/tool_exec_node.py
"""
...
from plan_and_then_execute_agent.shared.chat.tools import ToolCall
...
class _ToolExecSupportMixin:
    ...
    def _emit_tool_result(self, *, writer: Any | None, envelope: dict[str, Any]) -> None:
        if writer is None:
            return
        writer(
            {
    ...
    async def _sleep_backoff_async(self, *, backoffs: tuple[float, ...], attempt: int) -> None:
        delay = self._resolve_backoff_delay(backoffs=backoffs, attempt=attempt)
        if delay <= 0:
            return
        await asyncio.sleep(delay)

    def _resolve_backoff_delay(self, *, backoffs: tuple[float, ...], attempt: int) -> float:
        if not backoffs:
            return 0.0
    ...
    def _build_unexpected_flow_result(
        ...
        *,
        ...
        self._logger.error(
            ...
            f"tool_name={failure.get('tool_name')}, step_id={failure.get('step_id')}"
        )
