...
설명: ToolSpec 등록/조회/Planner 주입 스펙 생성을 공통화한다.
디자인 패턴: 레지스트리
...
from plan_and_then_execute_agent.shared.chat.tools.types import PlannerToolSpec, ToolFn, ToolSpec
...
class ToolRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        allowed_module_prefix: str = "plan_and_then_execute_agent.core.chat.tools",
    ) -> None:
    ...
    def add_tool(
        ...
        *,
        ...
        try:
            ...
            raise BaseAppException("Tool 등록 스펙이 올바르지 않습니다.", detail) from error
    ...
    def _validate_tool_module(self, *, fn: ToolFn, name: str) -> None:
        ...
        detail = ExceptionDetail(
            ...
            hint="Tool 함수 구현은 core/chat/tools 하위에 위치해야 합니다.",
        )
