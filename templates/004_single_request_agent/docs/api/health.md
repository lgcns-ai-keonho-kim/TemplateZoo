...
| Liveness | 프로세스가 살아 있는지 확인하는 관점 | `src/single_request_agent/api/health/routers/server.py` |
...
| 파일 | 역할 |
| --- | --- |
| `src/single_request_agent/api/health/routers/server.py` | `/health` 라우터 정의 |
| `src/single_request_agent/api/main.py` | health 라우터 앱 등록 |
...
1. `src/single_request_agent/api/health/routers`에 readiness 라우터 파일을 추가한다.
2. `src/single_request_agent/api/main.py`에 라우터 등록을 추가한다.
