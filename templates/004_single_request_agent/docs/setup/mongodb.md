...
| 경로 | 역할 |
| --- | --- |
| `src/single_request_agent/integrations/db/engines/mongodb/engine.py` | MongoDB 엔진 본체 |
| `src/single_request_agent/integrations/db/engines/mongodb/connection.py` | 연결/DB 객체 초기화 |
| `src/single_request_agent/integrations/db/engines/mongodb/schema_manager.py` | 컬렉션 생성/필드 변경 |
| `src/single_request_agent/integrations/db/engines/mongodb/filter_builder.py` | Query -> Mongo 필터 변환 |
| `src/single_request_agent/api/agent/services/runtime.py` | 저장소 조립 지점 |
| `src/single_request_agent/shared/agent/repositories/history_repository.py` | `db_client` 주입형 저장소 |
...
from single_request_agent.integrations.db.engines.mongodb import MongoDBEngine
from single_request_agent.shared.agent import ChatHistoryRepository
