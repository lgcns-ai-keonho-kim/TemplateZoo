...
디자인 패턴: 테스트 케이스
참조: src/single_request_agent/shared/runtime/queue/redis_queue.py, src/single_request_agent/shared/runtime/queue/model.py
"""
...
from single_request_agent.shared.runtime.queue import QueueConfig, RedisQueue
...
def test_redis_queue_put_get_and_close() -> None:
    ...
    if not url:
        raise RuntimeError("REDIS_URL 환경 변수가 필요합니다.")

    queue_name = f"unit-{uuid4().hex[:8]}"
    queue = RedisQueue(
        url=url,
        name=queue_name,
        config=QueueConfig(max_size=2, default_timeout=1),
    )
