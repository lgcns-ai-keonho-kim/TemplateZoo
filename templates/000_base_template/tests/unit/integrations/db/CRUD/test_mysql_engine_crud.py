"""
목적: MySQL 엔진의 기본 CRUD 동작을 검증한다.
설명: 실제 MySQL 환경에서 테이블/문서 작업을 확인한다.
디자인 패턴: 테스트 케이스
참조: src/base_template/integrations/db/engines/mysql.py
"""
...
from base_template.integrations.db.engines.mysql import MySQLEngine
...
def test_mysql_engine_basic_crud() -> None:
    """MySQL CRUD 기본 동작을 검증한다."""

    params = _mysql_params()
    if not params:
        pytest.skip("MYSQL_DSN_JSON 또는 MYSQL_* 환경 변수가 필요합니다.")

    _log_step("엔진 생성", mode="direct" if "dsn" not in params else "dsn")
    engine = MySQLEngine(**params)
    _log_step("클라이언트 생성")
...
def _mysql_params() -> dict | None:
    dsn_raw = os.getenv("MYSQL_DSN_JSON")
    ...
    if dsn_raw:
        dsn = json.loads(dsn_raw)
        port_value = dsn.get("port")
        if isinstance(port_value, str) and port_value.isdigit():
            dsn["port"] = int(port_value)
        return {"dsn": dsn}
    return None
