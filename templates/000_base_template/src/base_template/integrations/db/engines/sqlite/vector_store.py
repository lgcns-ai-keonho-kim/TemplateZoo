...
디자인 패턴: 저장소 패턴
참조: src/base_template/integrations/db/engines/sqlite/vector_codec.py
"""
...
from base_template.integrations.db.engines.sqlite.vector_codec import SqliteVectorCodec
...
class SqliteVectorStore:
    ...
    def create_vector_table(
        ...
        table = self._vector_table(schema)
        ...
        self._vector_tables[schema.name] = table

    def drop_vector_table(self, schema: CollectionSchema) -> None:
        """스키마 기준 벡터 테이블을 삭제한다."""
    ...
    def drop_vector_table_by_collection(self, collection: str) -> None:
        ...
        connection = self._connection_provider()
        cursor = connection.cursor()
        mapped_table = self._vector_tables.pop(collection, None)
        if mapped_table:
            cursor.execute(
                f"DROP TABLE IF EXISTS {self._identifier.quote_identifier(mapped_table)}"
            )
        cursor.execute(
    ...
    def delete_vector(self, schema: CollectionSchema, doc_id: object) -> None:
        """문서 벡터를 삭제한다."""

        if not self._enable_vector or not vector_field(schema):
            return
        connection = self._connection_provider()
        table = self._vector_table(schema)
        cursor = connection.cursor()
    ...
    def _vector_table(self, schema: CollectionSchema) -> str:
        return self._vector_tables.get(schema.name) or schema.vector_table or f"vec_{schema.name}"
